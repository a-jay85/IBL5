...
- A **delegation packet** (format in *Delegation packets for verbose phases* below) for every implementation phase that sweeps roughly **3+ file-edits** or runs a **run→inspect→fix→regen loop**. Packets are the lever that keeps the implementation orchestrator's context flat instead of marathoning into the ~100–150K dumb zone — yet audited 2026-07-07, only 12 of 235 corpus plans carried any. Treat a qualifying phase without a packet as an authoring defect, not a style choice.
