...
- Positive: the restored script fixes two latent defects in the retired version. It now checks the force-push exit status rather than assuming success (the old one printed `✓ Rebased` and incremented the counter even when `--force-with-lease` was rejected, so the summary could report rebases that never happened), and it normalizes the `dry_run` input so an unexpected rendering fails *safe* (skip the push) rather than open (push anyway).
