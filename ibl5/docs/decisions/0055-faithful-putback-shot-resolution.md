...
2. **3pt suppression (`jsb560_decompiled.c:94022-94024`).** 5.60 re-loops a 3pt outcome
   ...
   engine let `outcome3pt` be selected for a putback.
...
2. **3pt suppression** — the half-court trip loop (`possession.go`) zeroes the 3pt bucket
   ...
   is unchanged (one `Float64` + one `IntN` regardless of bucket weights).
...
diagnostic baseline. It gates BOTH divergences with the same polarity at both sites,
