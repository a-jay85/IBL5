...
> **Authoritative current schema:** `ibl5/docs/schema/current-schema.sql` holds the complete, always-current DDL (`000_baseline_schema.sql` + every applied migration), auto-regenerated by `bin/regen-schema-dump` on each schema change. Answer exact column/table/index questions from that file — do not walk `ibl5/migrations/` migration-by-migration, and do not trust the March `000_baseline_schema.sql` snapshot for current column names.
...
|---------|-------|------------|
| Players | `ibl_plr` | `pid`, `teamid`, `name`, `cy`, `cy1-cy6` |
...
### Team Roster
```php
$query = "SELECT * FROM ibl_plr WHERE teamid = ? ORDER BY ordinal";
```
