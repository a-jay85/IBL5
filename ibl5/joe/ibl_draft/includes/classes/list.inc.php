...
class table_list
{
    ...
    public function table_list($multipage = true, $interface_append = '')
    {
        ...
        // Retrieve the records per page, set it to 28 if not set
        ...
        if (!$this->records_per_page) {
            $this->records_per_page = 28;
        }
        setcookie('records_per_page_' . $this->interface_id, $this->records_per_page, time() + 60 * 60 * 28 * 360 * 10);
        $this->queries = array();
    ...
    public function append_query($queries)
    {
        // Add $queries to the end of the query to get back to this page
        $this->queries = array_merge($this->queries, $queries);
    }

    public function set_show_totals($value)
    {
    ...
    public function set_header($column, $text, $default_sort = '0', $link = true, $sort = true)
    {
    ...
    public function set_date_format($column, $format)
    {
        $this->header[$column]['date'] = $format;
    }

    public function set_link($link, $id_col)
    {
    ...
    public function set_data_format($column, $format)
    {
    ...
    public function set_number_format($column, $format)
    {
        $this->header[$column]['number_format'] = $format;
    }

    public function set_exec($column, $command)
    {
    ...
    public function set_form_extra($extra)
    {
        // Extra values for the form
        $this->form['extra'] = $extra;
    }

    public function set_form_button($name, $value, $type = "submit")
    {
        $this->buttons[$name]['value'] = $value;
        $this->buttons[$name]['type'] = $type;
    }

    public function set_map_data($lat, $long)
    {
        $this->latitude = $lat;
        $this->longitude = $long;
        $this->has_map = true;
    ...
    public function set_extra_text($text)
    {
    ...
    public function set_style($column)
    {
        // Pass a column to use as the contents of the "style" tag
        $this->style = $column;
    }

    public function set_cell_style($data_column, $style_column)
    {
...
<p class="report_total">Showing ' . number_format(($first + 1), 0) . ' - ' . number_format($last, 0) . ' of ' . number_format($this->rows, 0) . "</p>";
    ...
    public function draw_page_numbers()
    {
        $id = uniqid('form');
        $page = 1;
        $html = '
...
<option value="' . $page . '"' . $selected . '>' . $page . '</option>';
    ...
    <td class="pages">
      <select name="records_per_page" onchange="document.' . $id . '.submit()">';
        $options = array(5, 10, 28, 32, 50, 100, 200, 300, 400, 500, 1000, 2000, 5000);
