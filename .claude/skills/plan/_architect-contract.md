...
- **Backlog / GitHub Issue close** (only when Step 2 recorded a resolved GitHub Issue): the plan MUST include a bookkeeping step that closes the issue via `gh issue close <n> --repo a-jay85/IBL5-backlog -c '<what closed it>'`. Tier this step **Haiku (inline — one-shot `gh` CLI call, well below the ~15K bar)**.
