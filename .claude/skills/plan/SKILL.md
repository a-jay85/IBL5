...
**Worked example (the precedence in action):** the very plan that ADDED this Sonnet tier — the T11+T12 batch editing `/plan`'s ship pipeline — is itself *recipe-backed* (it names an exact recipe and a pattern to copy, `plan-architect.md`), which by check 2 alone would route to `plan-architect-sonnet`. But it also changes a `.claude/skills` ship-pipeline invariant, so **check 1 fires first** and it correctly went to `plan-architect-xhigh`, **not** Sonnet. That ordering is the load-bearing correctness property: recipe-backed must never downgrade a high-stakes plan.
