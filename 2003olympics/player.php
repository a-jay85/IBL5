...
<li><a href=\"player.php?id=$pid\">$name </a> (<a href=\"team.php?tid=$tid&yr=$year\">$team</a>)</li>";
    ...
    while ($i < $num) {
        $name = $db->sql_result($result, $i, "name");
        $year = $db->sql_result($result, $i, "draftyear") + $db->sql_result($result, $i, "exp");
        $tid = $db->sql_result($result, $i, "tid");
        $team = $db->sql_result($result, $i, "teamname");
        $pos = $db->sql_result($result, $i, "pos");
        $stats_gm = $db->sql_result($result, $i, "stats_gm");
        $stats_min = $db->sql_result($result, $i, "stats_min");
        $htft = $db->sql_result($result, $i, "htft");
        $htin = $db->sql_result($result, $i, "htin");
        $wt = $db->sql_result($result, $i, "wt");
        ...
        $skill = $db->sql_result($result, $i, "talent");
        $talent = $db->sql_result($result, $i, "skill");
        ...
        $draftround = $db->sql_result($result, $i, "draftround");
        ...
        $nation = $db->sql_result($result, $i, "nation");
...
/* =======================Loop player historical totals */
        ...
        while ($k < $num2) {
            ...
            $hist_year = $db->sql_result($result2, $k, "year");
            $hist_team = $db->sql_result($result2, $k, "team");
            $hist_gm = $db->sql_result($result2, $k, "gm");
            $hist_min = $db->sql_result($result2, $k, "min");
            $hist_fgm = $db->sql_result($result2, $k, "fgm");
            $hist_fga = $db->sql_result($result2, $k, "fga");
            @$hist_fgp = ($hist_fgm / $hist_fga);
            $hist_ftm = $db->sql_result($result2, $k, "ftm");
            $hist_fta = $db->sql_result($result2, $k, "fta");
            @$hist_ftp = ($hist_ftm / $hist_fta);
            $hist_tgm = $db->sql_result($result2, $k, "3gm");
            $hist_tga = $db->sql_result($result2, $k, "3ga");
            @$hist_tgp = ($hist_tgm / $hist_tga);
            $hist_orb = $db->sql_result($result2, $k, "orb");
            $hist_reb = $db->sql_result($result2, $k, "reb");
            $hist_ast = $db->sql_result($result2, $k, "ast");
            $hist_stl = $db->sql_result($result2, $k, "stl");
            $hist_tvr = $db->sql_result($result2, $k, "tvr");
            $hist_blk = $db->sql_result($result2, $k, "blk");
            $hist_pf = $db->sql_result($result2, $k, "pf");
            $hist_pts = $db->sql_result($result2, $k, "fgm") + $db->sql_result($result2, $k, "fgm") + $db->sql_result($result2, $k, "ftm") + $db->sql_result($result2, $k, "3gm");
...
/* ========== Loop player historical playoff totals ========== */
        ...
        while ($k < $num4) {
            ...
            $hist_year = $db->sql_result($result4, $k, "year");
            $hist_team = $db->sql_result($result4, $k, "team");
            $hist_gm = $db->sql_result($result4, $k, "games");
            $hist_min = $db->sql_result($result4, $k, "minutes");
            $hist_fgm = $db->sql_result($result4, $k, "fgm");
            $hist_fga = $db->sql_result($result4, $k, "fga");
            @$hist_fgp = ($hist_fgm / $hist_fga);
            $hist_ftm = $db->sql_result($result4, $k, "ftm");
            $hist_fta = $db->sql_result($result4, $k, "fta");
            @$hist_ftp = ($hist_ftm / $hist_fta);
            $hist_tgm = $db->sql_result($result4, $k, "tgm");
            $hist_tga = $db->sql_result($result4, $k, "tga");
            @$hist_tgp = ($hist_tgm / $hist_tga);
            $hist_orb = $db->sql_result($result4, $k, "orb");
            $hist_reb = $db->sql_result($result4, $k, "reb");
            $hist_ast = $db->sql_result($result4, $k, "ast");
            $hist_stl = $db->sql_result($result4, $k, "stl");
            $hist_tvr = $db->sql_result($result4, $k, "tvr");
            $hist_blk = $db->sql_result($result4, $k, "blk");
            $hist_pf = $db->sql_result($result4, $k, "pf");
            $hist_pts = $db->sql_result($result4, $k, "fgm") + $db->sql_result($result4, $k, "fgm") + $db->sql_result($result4, $k, "ftm") + $db->sql_result($result4, $k, "tgm");
...
";
            ...
            while ($c < $num4) {
                $college_year = $db->sql_result($result4, $c, "year");
                $college_gm = $db->sql_result($result4, $c, "games");
                $college_min = $db->sql_result($result4, $c, "minutes");
                $college_fgm = $db->sql_result($result4, $c, "fgm");
                $college_fga = $db->sql_result($result4, $c, "fga");
                @$college_fgp = ($college_fgm / $college_fga);
                $college_ftm = $db->sql_result($result4, $c, "ftm");
                $college_fta = $db->sql_result($result4, $c, "fta");
                @$college_ftp = ($college_ftm / $college_fta);
                $college_tgm = $db->sql_result($result4, $c, "tgm");
                $college_tga = $db->sql_result($result4, $c, "tga");
                @$college_tgp = ($college_tgm / $college_tga);
                $college_orb = $db->sql_result($result4, $c, "orb");
                $college_reb = $db->sql_result($result4, $c, "reb");
                $college_ast = $db->sql_result($result4, $c, "ast");
                $college_stl = $db->sql_result($result4, $c, "stl");
                $college_tvr = $db->sql_result($result4, $c, "tvr");
                $college_blk = $db->sql_result($result4, $c, "blk");
                $college_pf = $db->sql_result($result4, $c, "pf");
                $college_pts = $db->sql_result($result4, $c, "fgm") + $db->sql_result($result4, $c, "fgm") + $db->sql_result($result4, $c, "ftm") + $db->sql_result($result4, $c, "tgm");
...
/* =======================Loop player historical ratings */
        ...
        while ($kk < $numratings) {
            ...
            $r_year = $db->sql_result($resultratings, $kk, "year");
            $r_2ga = $db->sql_result($resultratings, $kk, "r_2ga");
            $r_2gp = $db->sql_result($resultratings, $kk, "r_2gp");
            $r_fta = $db->sql_result($resultratings, $kk, "r_fta");
            $r_ftp = $db->sql_result($resultratings, $kk, "r_ftp");
            $r_3ga = $db->sql_result($resultratings, $kk, "r_3ga");
            $r_3gp = $db->sql_result($resultratings, $kk, "r_3gp");
            $r_orb = $db->sql_result($resultratings, $kk, "r_orb");
            $r_drb = $db->sql_result($resultratings, $kk, "r_drb");
            $r_ast = $db->sql_result($resultratings, $kk, "r_ast");
            $r_stl = $db->sql_result($resultratings, $kk, "r_stl");
            $r_tvr = $db->sql_result($resultratings, $kk, "r_tvr");
            $r_blk = $db->sql_result($resultratings, $kk, "r_blk");
            $r_oo = $db->sql_result($resultratings, $kk, "r_oo");
            $r_do = $db->sql_result($resultratings, $kk, "r_do");
            $r_po = $db->sql_result($resultratings, $kk, "r_po");
            $r_to = $db->sql_result($resultratings, $kk, "r_to");
            $r_od = $db->sql_result($resultratings, $kk, "r_od");
            $r_dd = $db->sql_result($resultratings, $kk, "r_dd");
            $r_pd = $db->sql_result($resultratings, $kk, "r_pd");
            $r_td = $db->sql_result($resultratings, $kk, "r_td");
...
/* =======================END RATINGS */
    ...
    if ($num4 > 0) {
        while ($m < $num4) {
            $award_year = $db->sql_result($result4, $m, "year");
            $award_type = $db->sql_result($result4, $m, "Award");
