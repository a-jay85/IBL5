...
class TradeCashRepositoryTest extends TestCase
{
    ...
    public function testGetCashDetailsReturnsRowWhenFound(): void
    {
        $repository = new TradeCashRepository($this->mockMysqliDb);
        $this->mockDb->setMockData([
            ['tradeOfferID' => 1, 'sendingTeam' => 'Miami', 'receivingTeam' => 'Chicago', 'cy1' => 100, 'cy2' => 0, 'cy3' => 0, 'cy4' => 0, 'cy5' => 0, 'cy6' => 0],
        ]);

        $result = $repository->getCashDetails('Miami', 1);

        $this->assertIsArray($result);
        $this->assertSame('Miami', $result['sendingTeam']);
    }

    public function testGetCashDetailsReturnsNullWhenNotFound(): void
    {
        $repository = new TradeCashRepository($this->mockMysqliDb);
        $this->mockDb->setMockData([]);

        $result = $repository->getCashDetails('Nonexistent', 99);

        $this->assertNull($result);
    }

    // ============================================
    // INSERT POSITIVE CASH TRANSACTION TESTS
    // ============================================

    public function testInsertPositiveCashTransactionExecutesInsert(): void
    {
        $repository = new TradeCashRepository($this->mockMysqliDb);
        ...
        $result = $repository->insertPositiveCashTransaction($data);

        $this->assertIsInt($result);
    }

    // ============================================
    // INSERT NEGATIVE CASH TRANSACTION TESTS
    // ============================================

    public function testInsertNegativeCashTransactionExecutesInsert(): void
    {
        $repository = new TradeCashRepository($this->mockMysqliDb);
        ...
        $result = $repository->insertNegativeCashTransaction($data);

        $this->assertIsInt($result);
    }

    // ============================================
    // DELETE CASH TRANSACTION TESTS
    // ============================================

    public function testDeleteCashTransactionExecutesDelete(): void
    {
        $repository = new TradeCashRepository($this->mockMysqliDb);

        $result = $repository->deleteCashTransaction('Miami', 1);
