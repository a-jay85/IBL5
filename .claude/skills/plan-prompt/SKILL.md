...
- security surface / trust boundary / destructive migration / an **executable-gate
  removal or weakening** (the change lets an input a hook deny, a `bin/check-*`
  condition, or a Phase 6.5 arming condition previously rejected now pass) or a
  **bootstrap hazard** (the change rewrites the rule governing its own merge) →
  **`plan-architect-xhigh`**. Directory alone is never the trigger; full text at
  `/plan` Step 3 check 1.
...
resolve, but with low confidence, means you have not finished this step. Reopen it
with `AskUserQuestion` now, while the human is still in the room to answer.
...
2. Fire it — the file from step 1 is the argument:
   ...
   **Default disposition is `--queue`.** It matches `/plan`'s own default. The fired
   run goes design → plan → implementation → PR without a human in the loop, so the
   block you just composed is the last human-authored input to a shipped PR. The gate
   belongs to `bin/plan-now` itself: it queues only when the run **exits clean** *and*
   the plan is **identified by its `PLAN_FILE:` line** *and* **`bin/check-plan`
   passes**. A degraded run lands on disk for a human instead.

   Pass `--implement` only when the user asked, in this conversation, to read the plan
   before it implements. Nothing about the work itself selects it. The Step 4 tier
   triggers route the architect and have no bearing on disposition. Everything else
   queues safely because `/post-plan` always opens the PR, Phase 6.5's arming
   conditions only ever *hold*, and a `feat:` title stays held by the commit-type floor
   and the `human-signoff` required check regardless. A wrong queue call costs a PR you
   read at review time.
