...
**(15) Deferred meta-check hold.** If the pre-push local meta-check gate flagged failures that could not be fixed before pushing, or if re-running the checks against the real PR number still fails, hold auto-merge until the checks are green.

```bash
# condition (15): block if local meta checks failed pre-push or post-pr
