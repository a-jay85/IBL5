...
class OneOnOneGameEngine implements OneOnOneGameEngineInterface
{
    ...
    private function generateFinalScoreTable(OneOnOneGameResult $result): string
    {
        ...
        return "<table border=1><tr><td colspan=11><span style=\"color: #ff0000;\"><b>FINAL SCORE: {$result->player1Name} {$result->player1Score}, {$result->player2Name} {$result->player2Score}</b></span></td></tr>
<tr><td>Name</td><td>FGM</td><td>FGA</td><td>3GM</td><td>3GA</td><td>ORB</td><td>REB</td><td>STL</td><td>BLK</td><td>TVR</td><td>FOUL</td></tr>
<tr><td>{$result->player1Name}</td><td>{$p1Stats->fieldGoalsMade}</td><td>{$p1Stats->fieldGoalsAttempted}</td><td>{$p1Stats->threePointersMade}</td><td>{$p1Stats->threePointersAttempted}</td><td>{$p1Stats->offensiveRebounds}</td><td>{$p1Stats->totalRebounds}</td><td>{$p1Stats->steals}</td><td>{$p1Stats->blocks}</td><td>{$p1Stats->turnovers}</td><td>{$p1Stats->fouls}</td></tr>
<tr><td>{$result->player2Name}</td><td>{$p2Stats->fieldGoalsMade}</td><td>{$p2Stats->fieldGoalsAttempted}</td><td>{$p2Stats->threePointersMade}</td><td>{$p2Stats->threePointersAttempted}</td><td>{$p2Stats->offensiveRebounds}</td><td>{$p2Stats->totalRebounds}</td><td>{$p2Stats->steals}</td><td>{$p2Stats->blocks}</td><td>{$p2Stats->turnovers}</td><td>{$p2Stats->fouls}</td></tr>
</table>
";
    }
}
