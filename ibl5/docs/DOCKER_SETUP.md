...
```bash
# Canonical entry point — boots stack and streams a full prod sync
bin/dev-up

# Containers only (empty DB — populate manually with bin/db-sync-prod)
...
### Without prod credentials

Without `REMOTE_*` credentials in the repo-root `.env`, the live-sync path
(`bin/dev-up`, `bin/wt-up --prod`) fails fast. The credential-free alternatives:
...
- `docker compose up -d` — empty DB; populate manually with `bin/db-sync-prod`
