...
- **ADR auto-draft is one-shot and gated.** The arm fires before the Phase 2 commit. `_commit_with_adr_draft` runs `bin/adr-check --commit` with the branch's commit messages as the bypass body, and on a denial runs one bounded `claude-opus-5-5` call (Read, Grep, Glob, Write; no Bash) that fills a file allocated by `bin/next-adr`. The draft is staged, not separately committed, so it lands in the Phase 2 commit. The harness validates the file (frontmatter, harness attribution as the first body line, no `no-adr` text, no stray edits), renumbers a `bin/check-numbering` collision on the still-empty template, and runs `bin/check-numbering`, `bin/check-prose` and `bin/adr-check` locally; `bin/check-docs --since` is deferred at this site to the commit-gate doc-staleness remediation arm that runs immediately after. Any failure re-raises the original denial, so the run exits 3 with the draft named in the DM and the skill fallback stays suppressed. Stale-base denials are a push-time concept and cannot arise before the commit, which is the same structural guarantee by a shorter argument. `result.json` records `adr_drafted`, `adr_path` and `adr_draft_model`. `bin/post-plan-now` pins the main-checkout copy (ADR-0092), so the arm is live only after merge. `_push_with_adr_draft` remains at the Phase 5.5 push site as a backstop for a trigger introduced after the Phase 2 commit, and the `res.adr_drafted` guard keeps the total at one `claude-opus-5-5` call per run.
