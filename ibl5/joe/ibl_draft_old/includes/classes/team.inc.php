...
" . $order . " limit 1";
    ...
    public function new_pick_time($time)
    {
    ...
    public function lower_pick_limit()
    {
...
where team_id = '" . $this->data['team_id'] . "'";
    ...
    {
        // First see if we have autopick turned off
        // If so, see if there is something in the priority list we can use
        if (!$this->data['team_autopick']) {
            // Temporarily turn autopick on
            $this->data['team_autopick'] = 1;
        }
        $player_id = $this->next_player();
        if ($player_id) {
            return $player_id;
        }
...
('" . $mock_pick_id . "', '" . $this->data['team_id'] . "', '" . $row['player_id'] . "', '$commentary')";
    ...
    public function mock_commentary($row1, $row2)
    {
...
there really isn't a better choice at this point.";
    ...
    public function close_comment($row, $same_name)
    {
