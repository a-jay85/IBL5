---
description: Which tier to pick for each sub-agent, the two Sonnet 4.6 agent-def pins (Explore, automouse delegates), and how to tier Explore agents. Skip-vs-spawn heuristic and deeper rationale live in agent-tiering-detail.md.
...
| **Sonnet** | `model: "sonnet"` | Synthesis: "is this finding relevant?", cross-file traces, semantic compliance checks, rename sweeps needing call-site judgment. Two Sonnet-tier surfaces are instead **pinned to 4.6 by an agent def** — see § Sonnet 4.6 pins. |
...
Sonnet 5's tokenizer inflates every token a run charges against a subscription's budget (~30% tax), so the two **high-volume, mechanical** Sonnet surfaces are pinned to Sonnet 4.6 by an agent def. The `model` param is an **enum** (`sonnet|opus|haiku|fable`) and cannot express a specific version — a def's frontmatter is the only way to pin one, and **it wins only when the `model` param is omitted**.

| Surface | Def | Spawn with |
|---------|-----|-----------|
| **Explore** | `~/.claude/agents/Explore.md` (machine-local; shadows the built-in) | `subagent_type: "Explore"`, **omit `model`**. Passing `model: "sonnet"` is **blocked** by `~/.claude/hooks/explore-model-gate.sh`. |
| **Automouse impl delegates** | `.claude/agents/automouse-delegate.md` (in-repo) | `subagent_type: "automouse-delegate"`, **omit `model`**. Fired by `bin/automouse-prompt-impl` for each `### Delegate` packet at Sonnet tier. |

Everywhere else the Sonnet row's `model: "sonnet"` stands — a one-off synthesis agent isn't worth a def.
