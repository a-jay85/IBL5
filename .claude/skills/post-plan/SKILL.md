...
if [ "$MC_RC" = 0 ]; then
  ...
  echo "META-CHECKS=failed (${FAILED:-unknown}) — pushing anyway; condition (15) will hold auto-merge"
fi
```

3. push
4. **If no PR exists for the current branch:** create one with `gh pr create`. Read `.claude/skills/post-plan/_pr-body-claims.md` and apply it before writing the body. Read `.claude/review-shared/_prose-voice-contract.md` before writing any PR-body prose — it carries the five written-voice rules and the two annotated failure samples, and it governs every prose surface this phase and Phase 6 compose. <!-- slop-ok -->
...
# CONFLICT-REVIEW=FOUND-PROBLEM verdict is written to this same path, so a file-existence
# check would arm auto-merge on a negative review.
...
The compiled harness starts a `gh pr checks --watch` (no `--fail-fast`; it drops the red-by-design `human-signoff` check) the moment it pushes in
...
2. **Block until CI settles:** `gh pr checks <pr> --watch --interval 20`. Omit `--fail-fast`. `human-signoff` goes red within seconds on every `feat:` PR, so fail-fast returns there and never observes the real checks. (Bash timeout 1200000 = 20 min cap, which leaves a ~40-min cushion under `MAX_PP_SECS=3600` for Phase 5.0 conformance + Phases 8-11 cleanup). The gh CLI handles the polling and exit logic itself; do not re-implement it in jq. Exit codes: `0` = all checks passed, `8` = at least one failed, other = transport error.
