...
**Shellcheck verify.** Runs when shell scripts changed (mirrors CI job flags exactly). Skip if `! $HAS_SHELL`:

```bash
# Shellcheck verify — runs when shell scripts changed (mirrors CI job flags exactly)
if [ "${HAS_SHELL:-false}" = "true" ]; then
    echo "Running shellcheck over full corpus..."
    if ! bin/lib/shell-scripts.sh --full | xargs shellcheck --severity=warning --shell=bash \
         --exclude=SC2034,SC1090,SC2207 2>&1; then
        echo "MISSING: shellcheck failed — fix warnings before arming"
    fi
fi
```
...
Compiled-harness note: `tools/postplan-harness` computes conformance in-process (`harness/conformance.py` returns the unresolved list to `armable.py`) and never touches either `/tmp` file. It does, however, write the *same two signals* into its run dir at the end of its Phase 5.0: a `conformance-done` marker and a `missing-tests` bridge, kept as that run's audit trail. The marker is a shared contract across both writers, so keep them agreeing on the three-state meaning (marker absent = indeterminate = blocked). The `/tmp` paths above remain the fallback-path spelling and are what an in-session Phase 5.0 writes. Per ADR-0092 `bin/post-plan-now` pins the **main-checkout** harness, so any harness-side change here is inert until it merges to main-checkout `master`; until then the skill-path block above is the live writer.
...
3. Branch on the result:
   ...
   - **Exit 2** → run full suite: `bin/e2e-wt <worktree-name>`
   - **Exit 0, test file list on stdout** → run targeted: `bin/e2e-wt <worktree-name> <test-files-from-stdout>`
