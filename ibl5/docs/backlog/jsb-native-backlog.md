...
J23 round-half-up + base_time re-center (✅, 2026-07-16, #1495) — coupled faithful fix deferred from J21; ADR-0085 records the hold finding; shipped round-half-up + baseTimeMid re-center 14.5→13.65 (J23)
  └─→ J24 possession-clock subsystem port (◑ Partial 2026-07-19) — step classes + jitter SHIPPED; steal split + faithful shotValue2pt/3pt (D80/D60/D64/DE8, blockMod, flow term wired) SHIPPED (j24-mix-fixes PR); matchupQuality Phase 3 matched-defender term (+0xDC8 DefAST48 vs LeagueAST48ByPos) SHIPPED, non-matched term (+0x350) SHIPPED (J25 2026-07-18), Phase 4 usage-dominance-flag subsystem SHIPPED (J26 2026-07-19, jsb-phase4-usage-dominance-port — `computeUsageDominanceFlags` per-possession, both call sites wired); FG% measured 46.42% (Phase 4 fires only when usage ratio > 0.5, not triggered by real-world archive data); residuals: CEngine+0x30 flag, baseTimeMid walkback to 16.0, U{0..2} step class
...
**Status update (2026-07-19, Phase-4 usage-dominance-flag subsystem PORTED — jsb-phase4-usage-dominance-port branch):** `computeUsageDominanceFlags` now produces a `[6]bool` per team per possession (one flag per lineup slot 1–5, true = usage-dominant: per-possession 2PA-rate ratio > 0.5). Both call sites (`possession.go`, `transition.go`) compute and pass the flag arrays live to `matchupQuality`. Phase 4 accumulates positive `NonMatchedTerm` raw (no fatigue scaling) over defenders whose usage-dominance flag is set, gated off when the ball-handler's own flag is set. **Residual (4) is resolved.** Archive run confirms steal ∈ [8.0, 9.0]% (8.70% ✓) and indep-TO ∈ [4.4, 5.4]% (4.84% ✓) still pass; FG% measured 46.42% — Phase 4 is live but fires only when a player's per-possession 2PA ratio exceeds 0.5, which real-world archive lineups do not trigger, so the FG% is unchanged from the J25 baseline and the [47.5, 48.9]% band remains open. No constant tuned toward the band. Open residuals: CEngine+0x30 forced-redraw flag, baseTimeMid walkback to 16.0 post-share-closure, U{0..2} step-class taxonomy.
