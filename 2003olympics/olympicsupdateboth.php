...
echo "<HTML><HEAD><TITLE>OLYMPICS LEADERBOARD UPDATE (olympicsupdate.php and olympicslbupdate.php combined)</TITLE></HEAD><BODY>";

echo "<p>OLYMPICS LEADERBOARD UPDATE (olympicsupdate.php)<p>";

while ($i < $num1) {
    $playername = $db->sql_result($result1, $i, "name");
    $playerid = $db->sql_result($result1, $i, "pid");

    $query2 = "SELECT * FROM ibl_olympics_stats WHERE name LIKE '$playername'";
    ...
    $query3 = "DELETE FROM ibl_olympics_career_totals WHERE `name` = '$playername'";
    $result3 = $db->sql_query($query3);

    if ($tot_games > 0) {

        $query4 = "INSERT INTO ibl_olympics_career_totals (`pid` , `name` , `games` , `minutes` , `fgm` , `fga` , `ftm` , `fta` , `tgm` , `tga` , `orb` , `reb` , `ast` , `stl` , `tvr` , `blk` , `pf` , `pts` ) VALUES ( '$playerid' ,  '$playername' ,  '$tot_games' , '$tot_minutes' , '$tot_fgm' , '$tot_fga' , '$tot_ftm' , '$tot_fta' , '$tot_tgm' , '$tot_tga' , '$tot_orb' , '$tot_reb' , '$tot_ast' , '$tot_stl' , '$tot_tvr' , '$tot_blk' , '$tot_pf' , '$tot_pts' ) ";
...
echo "<p>OLYMPICS LEADERBOARD UPDATE (olympicslbupdate.php)<p>";
...
while ($i < $num1) {
    ...
    $query2 = "SELECT * FROM ibl_olympics_career_totals WHERE name LIKE '$playername'";
    $result2 = $db->sql_query($query2);
    @$num2 = $db->sql_numrows($result2);
