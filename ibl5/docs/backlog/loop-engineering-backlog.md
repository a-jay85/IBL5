...
| 2026-08-19 | #1921 | class: LLM output schema validator hard-fails on a benign reply-shape variation (envelope wrapper), producing a false safety signal instead of degrading gracefully | routed to: Rung 3 - new forced-trigger row in .claude/review-shared/_plan-verification.md (section: Forced integration-verification trigger): any plan adding or modifying a schema validator for LLM output in the harness must test that a common envelope variation degrades gracefully and does not post a false "no issues" signal | prior: -- |
