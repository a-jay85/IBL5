...
| E90 | Incomplete seam list in `bin/bug-pipeline-test-env` env block and `bin/bug-pipeline-e2e` guard causes production contact through unguarded tick seams | ✅ fixed this pass | — | S |
| E91 | Hardcoded main-checkout path in `ibl5/IBLbot/ecosystem.bugbot-test.config.cjs` splits precondition check from PM2 artifact | ✅ fixed this pass | — | XS |
| E92 | PR body carried stale ADR number (0108 vs 0111) and false no-manual-testing claim | ✅ fixed this pass | — | XS |
| E93 | `REPLACE_WITH_` sentinel passes the non-empty approver-ID guard in `bin/bug-pipeline-test-env` | ✅ fixed this pass | — | XS |
...
### E69 PR #1899 Phase 6.5 — stale test-row labels and PR body claims when rows renumbered post-impl
...
| # | File:line | Same class? | Live? | Status |
|---|-----------|-------------|-------|--------|
| 1 (F1) | `bin/test-bug-pipeline-hunt:9` — header row list named `41,42,43,44,45,46,47`; rows were renumbered to `50–56` during implementation | yes | yes | fixed this pass |
...
- **rung 1 — extend an existing gate?** **Yes, and cheaply — but the caveat must NOT go inside the `### Merge digest` block.** Give the digest the same treatment `_phase7-verdict.md` already gives files-changed: when Phase 6.5 pushed any remediation commit, the Phase 7 composer states the SHA the digest actually describes. Carry it on the **provenance line above the first heading** (landed by #2154), extending that line to `covers head <sha> · digest basis <sha> · master <sha> · written <utc>`. This preserves the verbatim-paste contract (the five lines are untouched), needs no new script, no second Phase 6 spawn, and sits outside every parser. Pair it with a case-28 assertion binding the `digest basis` field to a non-empty remediation section.

  **correction (2026-09-06) — the originally-filed mechanism was unsafe.** This entry first proposed an unbolded `digest-basis:` line placed *under* the `### Merge digest` heading. That would silently corrupt the merge-digest ledger. `bin/pr-cycle`'s `_digest_labels` (grep the symbol) treats the region between that heading and the next heading or horizontal rule as the digest block, and folds any line lacking a `**Label:**` prefix onto the **preceding label's value** — so the caveat would be appended to `**Machine-authored fixes:**` and `_digest_row` would emit a mangled ledger row. Adding it as a bolded sixth label is equally closed: `scripts/digest.sh` and `bin/test-pr-ready-now` case 28 both pin the block at exactly five labelled lines. The only safe home is above the first heading, which is why the rung now points at the provenance line. Discovered while implementing that line in #2154; the digest-basis field itself was deliberately left out of #2154 and remains open here.
...
- **landing rung:** **rung 1** — add a `digest basis <sha>` field to the Phase 7 **provenance line** (the body's first line, above every heading), mirroring the files-changed refresh-line precedent, plus a `bin/test-pr-ready-now` case-28 assertion binding the field to a non-empty remediation section. Placement is load-bearing, not stylistic — see the correction above before implementing.

`prevention_ladder: rung 1 — Phase 7 carries the digest basis SHA on the provenance line above the first heading when 6.5 pushed remediation commits; case-28 assertion binds it`
...
### E64 Duplicate `last_verified:` frontmatter keys in `ibl5/docs/decisions/README.md`
...
### E90 Incomplete seam list in `bin/bug-pipeline-test-env` env block and `bin/bug-pipeline-e2e` guard causes production contact through unguarded tick seams
...
### E91 Hardcoded main-checkout path in `ibl5/IBLbot/ecosystem.bugbot-test.config.cjs` splits precondition check from PM2 artifact
...
### E92 PR body carried stale ADR number (0108 vs 0111) and false no-manual-testing claim
...
### E93 `REPLACE_WITH_` sentinel passes the non-empty approver-ID guard in `bin/bug-pipeline-test-env`
...
### E94 No committed regression tests for guards in `bin/bug-pipeline-e2e` and `ecosystem.bugbot-test.config.cjs` fail-closed checks
...
### E95 Unasked-for `last_verified` change with duplicate YAML key in `ibl5/docs/decisions/README.md`
...
### E96 Three residual isolation/scope gaps in `bin/bug-pipeline-test-env` (F3/F6/F7 from #1950 Phase 6)
