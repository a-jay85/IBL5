...
**Manual testing in PR description:** Include a "Manual Testing" section with a markdown checklist (`- [ ]`) of steps to verify functionality not covered by PHPUnit, PHPStan, or Playwright. Thorough enough to confirm the plan's goals are met, but minimal — no redundancy with automated checks.
