...
**Table evidence (2026-07-25):** `RecordFormatter` collaborator extracted (#1167, merged 2026-06-21); stubs/cache narrowed earlier (−39 LOC). Service ~687 LOC = residual typed-getter/query bulk, not this finding's concern.
...
**Table evidence (2026-07-25):** `StreakCalculator` exists (#1040); HEAT CTE → `vw_heat_champions` (migration 149, #1090). Repo still 823 LOC = separate cohesive batch-query concern, not this finding.
...
**Table evidence (2026-07-25):** `AnnouncementDispatcherInterface` injected (defaulted to `DiscordAnnouncementDispatcher`); dispatch loop now isolates per-message failures; `NullAnnouncementDispatcher` for dry-run/tests. Green-green + 1 tested resilience delta.
...
**Table evidence (2026-07-25):** `JsbImportService` now 177-LOC facade + 10 `Importers/` (#801).
...
**Table evidence (2026-07-25):** #1148 — 3 collaborators extracted; service ~386 LOC.
...
**Table evidence (2026-07-25):** Dual-path unified, byte-identical (#1146). View still 610 LOC = residual size, not this concern.
...
**Table evidence (2026-07-25):** `TeamPageDataPreparer`+`TeamCardRenderer` (#1144); `TeamService` ~115 LOC (absent from >500 list).
...
**Table evidence (2026-07-25):** Verified: no inline `new TeamQueryRepository`/`BuyoutLedgerRepository` in the view. Still 590 LOC = residual.
...
**Table evidence (2026-07-25):** Memoized active-DC fetch (`getActiveDc()`) + injected `SavedDepthChartRepositoryInterface`; green-green.
...
**Table evidence (2026-07-25):** Verified: `syncPropertiesFromPlayerData` gone, zero public nullable props. `Player.php` 671 LOC = typed-getter bulk (residual).
...
**Table evidence (2026-07-25):** `insertTeamBoxscore()` 30-arg signature now lives in `BoxscoreRepository:252`; 557 LOC. Array-param + injectable ProgressReporter; pin with a DB-integration test.
...
**Table evidence (2026-07-25):** #1145 — split into Index/Detail views, byte-identical golden-master.
...
**Table evidence (2026-07-25):** Mutable `$collectedPlayerNames` accumulator → local var passed by-ref; green-green; getAllSeasons() untouched.
...
**Table evidence (2026-07-25):** `TradeCapCalculator` extracted (#1143).
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — `YourAccountView` is now 122 LOC; extraction already done.
...
**Table evidence (2026-07-25):** `computeJsbProduction()` moved to service (verified).
...
**Location:** `ibl5/classes/Updater/StandingsUpdater.php`
**Problem:** `update()`, `computeAndInsertStandings()` and `computeAndInsertAll()` used `echo` for progress, captured by `UpdateStandingsStep::execute()` via `ob_start`/`ob_get_clean` into `StepResult::capturedLog`; two `$log` accumulators rendered to admins via `\UI\DebugOutput::display()`.
**Status:** Completed — echoes migrated to a private `$outputBuffer` on `StandingsUpdater` behind a `protected appendOutput()` seam, drained by the step via `takeOutputBuffer()`; `ob_start`/`ob_get_clean` removed from `UpdateStandingsStep`. GM-facing `capturedLog` is byte-identical (pinned by a permanent characterization test). Three admin-only `DebugOutput` panels were deliberately removed ('Computed Standings', the six per-region magic-number logs, and 'Computed Olympics Standings'). Scope expanded beyond the original two files to `OlympicsFlatStandingsUpdater`, which overrides `update()`/`computeAndInsertAll()` and flows through the same step. A source-level guard test (`tests/UpdateAllTheThings/StandingsUpdaterRegressionGuardTest.php`) keeps `echo` out of both updaters and `ob_start` out of the step, since `BanEchoInNonViewClassesRule` exempts the whole `*Updater.php` suffix.

**Table evidence (2026-08-08):** StandingsUpdater. `82`→`League::REGULAR_SEASON_GAMES` DONE (refactor PR). echo→logger + `$log` removal DEFERRED: behavior-changing — echoes feed the rendered pipeline `capturedLog` (UpdateStandingsStep→UpdaterController); `$log` feeds admin-rendered `DebugOutput::display`. Needs a non-`refactor:` PR.
...
**Table evidence (2026-07-25):** **Status (2026-06-28):** Declined. ActivityTracker + AllStarAppearances `index.php` are single-repo R+V passthrough (one `repo->getX()` → `view->render()`, verified on disk). A Service would be single-implementor pass-through ceremony — no orchestration to house (cf. 2.5/2.8/2.3 passthrough declines).
...
**Table evidence (2026-07-25):** **Status (2026-06-28):** Declined for GMContactList. Its `index.php` is single-repo passthrough (`getAllTeamContacts()` → render). The "match Topics (#1030)" premise is invalid: `TopicsService` orchestrates **two** repos (Topics + Search → `getPageData`) — real aggregation, not uniformity. No equivalent need here.
...
**Table evidence (2026-07-25):** DraftService + DraftController extracted; index.php globals removed; green-green.
...
**Table evidence (2026-07-25):** InjuriesRepository extracted+injected (#970).
...
**Table evidence (2026-07-25):** Search + Standings: pass-through Service = dead code; declined.
...
**Table evidence (2026-07-25):** `*ApiHandler` is the HTMX-fragment convention (9 handlers); rename declined.
...
**Table evidence (2026-07-25):** RookieOptionFormView→RookieOptionView (#1031); Service declined.
...
**Table evidence (2026-07-25):** OfferView already canonical (4.22); DemandsBreakdownView moved to Negotiation\Views\ sub-view (this PR). Calculator role tracked under 4.8.
...
**Table evidence (2026-07-25):** **Marker was missing/stale.** `sanitizeRedirect`+`toggleExtensions` moved to `Debug\DebugController`; index.php uses it (verified).
...
**Table evidence (2026-07-25):** News R/S/V extracted: `Topics\News\NewsRepository`, `NewsService`, `NewsView` (PR #1166, merged 2026-07-03).
...
**Table evidence (2026-07-25):** **Marker was missing/stale.** `modules/SiteStatistics/` deleted (#733, verified absent).
...
**Table evidence (2026-07-25):** ScheduleController owns the branch (this PR); index.php thin; league + team pages byte-identical (E2E).
...
**Table evidence (2026-07-25):** `Services/` deleted (2026-05-16).
...
**Table evidence (2026-07-25):** `Shared/` deleted; SalaryConverter→BasketballStats.
...
**Table evidence (2026-07-25):** NavigationViewInterface + 4 sub-view interfaces added; views implement them (this PR).
...
**Table evidence (2026-07-25):** Documented Updater web-POST placement (`classes/Updater/README.md`); audit's "CLI-only" was inaccurate — it is web-only.
...
**Table evidence (2026-07-25):** TeamStatsCalculator + StrengthOfScheduleCalculator moved into BasketballStats\ (this PR); old single-class dirs removed.
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — `DebugSession::__construct(?string $username, ?string $serverName, ?string $cookieValue, bool $isE2ETesting)` takes explicit params; `DebugController` reads superglobals and passes them in (lines 29–34).
...
**Table evidence (2026-07-25):** Folded into `index.php?op=select`.
...
**Table evidence (2026-07-25):** Folded into `?op=submit_asg
...
**Table evidence (2026-07-25):** `save_order.php`→`?op=save_order` (2026-06-05).
...
**Table evidence (2026-07-25):** `view.php` inlined; deleted.
...
**Table evidence (2026-07-25):** Hardened via `Auth\DemoLoginGate`, fails closed (ADR-0034).
...
**Table evidence (2026-07-25):** `configOlympics.php` deleted.
...
**Table evidence (2026-07-25):** mainfile.php dedup → `require LegacyFunctions` (ADR-0030).
...
**Table evidence (2026-07-25):** `$_REQUEST`→`$GLOBALS` block removed; 2-key allowlist.
...
**Table evidence (2026-07-25):** #1008 — block.php: no inline `<style>`/`number_format` (verified).
...
**Table evidence (2026-07-25):** #1008 — strict_types on mainfile/modules/index (verified).
...
**Table evidence (2026-07-25):** gzip/MSIE dead code gone.
...
**Table evidence (2026-07-25):** #1008 — example carries admin_file/IBL6_BASE_URL/DEMO_LOGIN_TOKEN (verified).
...
**Table evidence (2026-07-25):** index.php thin shim.
...
**Table evidence (2026-07-25):** Bootstrap\Application is composition root (ADR-0030).
...
**Table evidence (2026-07-25):** Shipped: `require __DIR__ . '/mainfile.php'`.
...
**Table evidence (2026-07-25):** #1008 — test-state.php uses `prepare()`+`bind_param` (verified).
...
**Table evidence (2026-07-25):** mainfile.php `filter()`/`addslashes` save=1 path gone.
...
**Table evidence (2026-07-25):** modules.php reads `$_GET['name']` w/ regex + `===`.
...
**Table evidence (2026-07-25):** Convention already documented at `ibl5/classes/Trading/README.md` (canonical home); cross-linked from `ibl5/docs/ARCHITECTURE_PATTERNS.md` (2026-06-22). Rename sweep explicitly out of scope.
...
**Table evidence (2026-07-25):** CashConsideration→`BuyoutLedgerRepository` (2026-05-20).
...
**Table evidence (2026-07-25):** `Services/` deleted.
...
**Table evidence (2026-07-25):** `Shared/SharedRepository` deleted.
...
**Table evidence (2026-07-25):** →FreeAgencyOfferView (2026-05-17).
...
**Table evidence (2026-07-25):** Renamed: `FreeAgencyMarketDemandCalculator` + `ExtensionContractDemandCalculator`.
...
**Table evidence (2026-07-25):** `*ApiHandler` (module-local HTMX) vs `Api\Controller\*Controller` (REST) documented in `ibl5/docs/ARCHITECTURE_PATTERNS.md` (2026-06-22).
...
**Table evidence (2026-07-25):** Renamed `TradeQueueProcessor` → `NightlyTradeBatchRunner`; no production caller.
...
**Table evidence (2026-07-25):** car_to→car_tvr (migration 128).
...
**Table evidence (2026-07-25):** car_tgm→car_3gm (migration 128).
...
**Table evidence (2026-07-25):** Two-letter rating cols documented (#1039); `r_*` rename deferred (L).
...
**Table evidence (2026-07-25):** cy/cyt documented (#1039); rename deferred.
...
**Table evidence (2026-07-25):** bird/exp documented (#1039); rename deferred.
...
**Table evidence (2026-07-25):** sh_/sp_/ch_/cp_ prefixes documented (#1039).
...
**Table evidence (2026-07-25):** Migration 121 added COMMENT to `game_2gm`/`game_2ga` on both `ibl_box_scores` and `ibl_box_scores_teams` (live DB confirmed 2026-06-28).
...
**Table evidence (2026-07-25):** dc_* codes documented (#1039).
...
**Table evidence (2026-07-25):** →NegotiationOfferView.
...
**Table evidence (2026-07-25):** →VotingResultsView.
...
**Table evidence (2026-07-25):** InjuriesRepository extracted (#970) (= 2.7).
...
**Table evidence (2026-07-25):** →`Topics\News\NewsRepository` (ADR-0001).
...
**Table evidence (2026-07-25):** PreseasonCleanupRepository moved to Updater\ root (this PR); a Repository, not a Step.
...
**Table evidence (2026-07-25):** class.notFound 293→7 (2026-05-16).
...
**Table evidence (2026-07-25):** Trading staticMethod.dynamicCall 0 (via 5.1).
...
**Table evidence (2026-07-25):** UAT 151 class/method.notFound gone (via 5.1).
...
**Table evidence (2026-07-25):** MockDatabase argument.type 0 (#1028).
...
**Table evidence (2026-07-25):** SeasonLeaderboardsView 71 XSS cleared; zero-floor.

### 5.6 — Finding 5.6
**Status:** ✅ Implemented — CareerLeaderboardsView 44 cleared.
**Table evidence (2026-07-25):** CareerLeaderboardsView 44 cleared.

### 5.7 — Finding 5.7
**Status:** ✅ Implemented — Navigation views 58 cleared; zero-floor.
**Table evidence (2026-07-25):** Navigation views 58 cleared; zero-floor.

### 5.8 — Finding 5.8
**Status:** ✅ Implemented — YourAccountView 27 cleared.
**Table evidence (2026-07-25):** YourAccountView 27 cleared.
...
**Table evidence (2026-07-25):** Gated (ADR-0032); bootstrap superglobal reads allowlisted by design.
...
**Table evidence (2026-07-25):** missingType.return 0 (#939/#958).
...
**Table evidence (2026-07-25):** assertEquals→assertSame 0 (#940/#958).
...
**Table evidence (2026-07-25):** JsbParser 25 method.notFound cleared.
...
**Table evidence (2026-07-25):** CommonMysqliRepositoryInterface (7.1) cleared 27.
...
**Table evidence (2026-07-25):** constructor.missingParentCall 0.
...
**Table evidence (2026-07-25):** ibl.inlineCss 0; PageLayout inline `@phpstan-ignore`.
...
**Table evidence (2026-07-25):** cookieBeforeHeader 4 cleared; zero-floor (ADR-0032).
...
**Table evidence (2026-07-25):** missingType.iterableValue 0 (#939/#958).
...
**Table evidence (2026-07-25):** ibl.deprecatedHtmlTag 0 in both baselines.
...
**Table evidence (2026-07-25):** TeamStatsCalculatorTest exists.
...
**Table evidence (2026-07-25):** StrengthOfScheduleCalculatorTest exists.
...
**Table evidence (2026-07-25):** LeagueStarters thin; additive.
...
**Table evidence (2026-07-25):** ApiKeysRepositoryTest added (revocation active-only scoping, latest-key lookup).
...
**Table evidence (2026-07-25):** ContractListServiceTest extended (#1161, 2026-06-20).
...
**Table evidence (2026-07-25):** FreeAgencyPreview thin; additive — coordinate with PR #1162 (future-year restore).
...
**Table evidence (2026-07-25):** SeasonHighs thin; additive.
...
**Table evidence (2026-07-25):** VotingRepositoryTest + SubmissionResultTest added (aggregation, column allowlist).
...
**Table evidence (2026-07-25):** Interface extracted; 34 sites inject (2026-05-16).
...
**Table evidence (2026-07-25):** Bare instantiation banned by `ibl.directCommonMysqliInstantiation`.
...
**Table evidence (2026-07-25):** PlayerViewFactory `$GLOBALS` fallback removed.
...
**Table evidence (2026-07-25):** getPlayerIdsByNames→`fetchAllInList()`.
...
**Table evidence (2026-07-25):** DI sweep injects interface.
...
**Table evidence (2026-07-25):** Split into TeamIdentity/PlayerLookup/SalaryCap.
...
**Table evidence (2026-07-25):** Waivers interface shapes (#1032).
...
**Table evidence (2026-07-25):** TeamOrderBy enum (#1032).
...
**Table evidence (2026-07-25):** DraftRepository injects TeamIdentityRepositoryInterface.
...
**Table evidence (2026-07-25):** Extracted `Standings\PythagoreanCalculator` (pure `calculate()`); StandingsRepository delegates both call sites; green-green.
...
**Table evidence (2026-07-25):** 8 map*/FIELD_MAP members (verified); PlayerRepository 622 LOC. Extract PlayerDataMapper/Hydrator; green-green.
...
**Table evidence (2026-07-25):** Hidden caches dropped (#1040).
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — `@see nuke_stories` annotation present at `PlayerRepository.php:216`.
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — `PlayerTeamJoinQuery` trait at `classes/Repositories/PlayerTeamJoinQuery.php:12`; used by `PlayerLookupRepository.php:14` and `PlayerRepository.php:28`.
...
**Table evidence (2026-07-25):** Dumps absent from `git log --all --full-history`; `.git` pack 279M (not 1.1GB) → clone-weight resolved. Credential angle: rotated + scrubbed from HEAD + gitleaks gate (ADR-0034); git-**history rewrite deliberately declined there** (rotation is the remediation), so the literal still lives in history by design.
...
**Table evidence (2026-07-25):** classes/Scripts → classes/Maintenance namespace rename (16 callers swept) (this PR).
...
**Table evidence (2026-07-25):** Document symlink strategy (docs). **Status:** Added `## Symlink strategy` to `bin/README.md` stating the convention (single tracked symlink `bin/db-query` → `ibl5/bin/db-query`; real files pinned to `ibl5/bin/` by the `./ibl5` Docker bind-mount; new symlinks discouraged) (this PR).
...
**Table evidence (2026-07-25):** `shellScripts/.env` not in working tree or history (verified); prod cred rotated + gitleaks scanning gate (ADR-0034). Per ADR-0034 the already-leaked literal is allowlisted, **not** purged from history (rewrite declined as too disruptive).
...
**Table evidence (2026-07-25):** Documented the de-facto check-script exit-code/output standard in `bin/README.md`. Helper lib deferred (separate item).
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — ShellCheck bash-3.2-compat job exists in `.github/workflows/tests.yml` (line 370+).
...
**Table evidence (2026-07-25):** Counts refreshed, ibl_plr_chunk/MyISAM removed (catchup).
...
**Table evidence (2026-07-25):** PostgreSQL section removed.
...
**Table evidence (2026-07-25):** docs/README index reflects built API.
...
**Table evidence (2026-07-25):** Inline test counts removed.
...
**Table evidence (2026-07-25):** Power_Rankings removed; count→7.
...
**Table evidence (2026-07-25):** "Established" aligned to Waivers.
...
**Table evidence (2026-07-25):** **Marker was missing/stale.** The stale GitHub-skills path is no longer referenced in DEVELOPMENT_GUIDE; skills live at `.claude/skills/` (verified).
...
**Table evidence (2026-07-25):** Copilot retired/archived (2026-06-10).
...
**Table evidence (2026-07-25):** DOCUMENTATION_STANDARDS lists both SECURITY.md exemplars (DepthChartEntry + ComparePlayers) + notes it is not a per-module requirement; docs-only.
...
**Table evidence (2026-07-25):** Archive scope (#1044).
...
**Table evidence (2026-07-25):** Schema Version line removed.
...
**Table evidence (2026-07-25):** css-architecture path-conditional rule verified accurate (paths: frontmatter scopes it); no change needed.
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-07-24, no plan — stale-Open) — both `lighthouse-pr-comments.md` and `refactor-flag.md` now carry `paths:` frontmatter.
...
**Table evidence (2026-07-25):** REFACTORING_HISTORY archived (#1044).
...
**Table evidence (2026-07-25):** PLR_VS_BOXSCORES hook added (#1044).
...
**Table evidence (2026-07-25):** Coverage figures aligned (~80%/70%).
...
**Table evidence (2026-07-25):** Added READMEs to all 68 class dirs that lacked one (2026-07-24); all new files carry doc-freshness frontmatter.
...
**Table evidence (2026-07-25):** Added frontmatter to the 16 existing class READMEs that lacked it; extended bin/check-docs IN_SCOPE_GLOBS to include ibl5/classes/*/README.md (2026-07-24).
...
**Table evidence (2026-07-25):** migrations/README stale FK removed (#1044).
...
**Table evidence (2026-07-25):** Opportunistic-backfill README policy added to DOCUMENTATION_STANDARDS; docs-only.
...
**Table evidence (2026-07-25):** STRATEGIC_PRIORITIES Completed-drops subsection.
...
**Table evidence (2026-07-25):** NegotiationServiceInterface PHPDoc added (#1044).
...
**Table evidence (2026-07-25):** Baseline hygiene (#1028).
...
**Table evidence (2026-07-25):** `_SESSION` banned (rule expansion).
...
**Table evidence (2026-07-25):** `_SERVER` banned.
...
**Table evidence (2026-07-25):** `BanEchoInNonViewClassesRule` landed; baseline burndown complete — 0 `ibl.echoInNonView` entries remain (verified 2026-07-24).
...
**Table evidence (2026-07-25):** `BanGlobalKeywordRule` landed.
...
**Table evidence (2026-07-25):** `GLOBALS` covered by superglobal rule.
...
**Table evidence (2026-07-25):** `BanDieExitInProductionRule` (0 baseline).
...
**Table evidence (2026-07-25):** `BanCastFunctionsRule` (0).
...
**Table evidence (2026-07-25):** `BanRawHtmlEscapeFunctionsRule` (0).
...
**Table evidence (2026-07-25):** `RequireTrustedAnnotationRule` (`ibl.trustedVariable`) guards `HtmlSanitizer::trusted()`; 147 occurrences across 16 file entries baselined (green-green), ADR-0077.
...
**Table evidence (2026-07-25):** `BanDirectHeaderCallRule` (1 baseline).
...
**Table evidence (2026-07-25):** `BanDirectMysqliQueryRule` landed; 7 Updater-step sites — burndown 🟩. **Status:** baseline cleared — 7 sites in 3 Refresh steps rerouted to `BaseMysqliRepository::execute()`.
...
**Table evidence (2026-07-25):** `BanSqlStringInterpolationRule` landed; all sqlInterp baseline entries cleared (PR2 finisher).
...
**Table evidence (2026-07-25):** Clock abstraction + `BanDirectTimeCallsRule` (PR #1160, merged 2026-06-28). `ClockInterface` at `classes/Clock/ClockInterface.php`.
...
**Table evidence (2026-07-25):** Removed `ob_start()`/`echo ob_get_clean()` wrapper; output byte-identical (characterization-pinned).

### 10.16 — Finding 10.16
**Status:** ✅ Implemented — unescapedOutput baseline cleared.
**Table evidence (2026-07-25):** unescapedOutput baseline cleared.
...
**Table evidence (2026-07-25):** cookieBeforeHeader cleared (ADR-0032).
...
**Table evidence (2026-07-25):** `BanServiceExtendsBaseRepositoryRule` (0).
...
**Table evidence (2026-07-25):** `BanDuplicateModifierMethodRule` landed; 5 ExtensionOfferEvaluator sites — burndown 🟩. **Status:** baseline cleared — 5 adapter methods renamed `calculate*Modifier`→`compute*Modifier` (delegation to `ContractRules` unchanged).
...
**Table evidence (2026-07-25):** `RequireMeaningfulAssertionsRule` now flags `assertNotNull()` on statically-non-null values via `$scope->getType()->isNull()`. **Status:** conservative `isNull()->no()` gate flagged zero existing sites — no baseline change.
...
**Table evidence (2026-07-25):** `BanBareColumnIdentifierRule` landed; all baseline sites cleared (shared-file residual cleared in PR2).
...
**Table evidence (2026-07-25):** `BanJsonDecodeWithoutThrowFlagRule` (0).
...
**Table evidence (2026-07-25):** `BanHardcodedEnvironmentStringsRule` landed; 8 sites — config-injection burndown 🟩 (env-branching — verify behavior).
...
**Table evidence (2026-07-25):** RequireStrictTypesRule scope extended.
...
**Table evidence (2026-07-25):** CSS orphan cleanup (#1027).
...
**Table evidence (2026-07-25):** Standalone-head font helper (#1027).
...
**Table evidence (2026-07-25):** editor.css deleted (#1027).
...
**Table evidence (2026-07-25):** menu GIFs deleted (#1027).
...
**Table evidence (2026-07-25):** dual-token convention documented (#1027).
...
**Table evidence (2026-07-25):** **Status:** 3 depth-chart CSS files → 1 (`depth-chart.css`); 2 satellites deleted, single `@import` placed after `tables.css`. VR-pinned pixel-identical (existing baseline unchanged).
...
**Table evidence (2026-07-25):** themecenterbox content-sniff replaced with explicit `$type` param (#1232, merged 2026-06-28).
...
**Table evidence (2026-07-25):** display:none→hidden cleared.
...
**Table evidence (2026-07-25):** TradingView `str_replace` CSS-patch replaced with explicit render param (#1232, merged 2026-06-28).
...
**Table evidence (2026-07-25):** gitignore clarifying comment added.
...
**Table evidence (2026-07-25):** `*.sch` gitignore + `!`-exceptions.
...
**Table evidence (2026-07-25):** IBL5.lge kept tracked (test fixture); relocation deferred (🟩 if pursued).
...
**Table evidence (2026-07-25):** tests-baseline reduced 9519→7557 (2026-05-16).
...
**Table evidence (2026-07-25):** Thumbs.db git rm'd + ignored.
...
**Table evidence (2026-07-25):** coverage-baseline.json intentionally tracked (CI consumes; ADR-0018).
...
**Table evidence (2026-07-25):** phpunit-baseline.xml intentionally tracked (phpunit.xml references it).
...
**Table evidence (2026-07-25):** PlayerSeasonTableRenderer extracted.
...
**Table evidence (2026-07-25):** Merged into 13.1.
...
**Table evidence (2026-07-25):** **Status:** Done (verified 2026-06-28, no plan — stale-Open) — `BaseMysqliRepository::gameOfThatDaySubquery()` (the protected-helper option of the suggested direction) is used by all 5 sites; zero inline `MIN(game_of_that_day)` copies remain.
...
**Table evidence (2026-07-25):** #1033.
...
**Table evidence (2026-07-25):** retired filter standardized to `= 0` (2026-06-05).
...
**Table evidence (2026-07-25):** Premise invalid: Career/Season `processPlayerRow` return arrays differ in key membership, order, and value-type (22 vs 44 keys) — no shared surface to extract. Doc-only closure, no code.
...
**Table evidence (2026-07-25):** cleanName subquery extracted (2026-06-05).
...
**Table evidence (2026-07-25):** Application wired (ADR-0030).
...
**Table evidence (2026-07-25):** Duplicate bootstrap removed.
...
**Table evidence (2026-07-25):** LegacyFunctions single source.
...
**Table evidence (2026-07-25):** Three modes via Factory.
...
**Table evidence (2026-07-25):** api.php via ApiApplicationFactory.
...
**Table evidence (2026-07-25):** Season DI (18 sites) #1096.
...
**Table evidence (2026-07-25):** LoggerInterface DI burndown (#1093/#1094/#1095).
...
**Table evidence (2026-07-25):** teamID→teamid (migration 114).
...
**Table evidence (2026-07-25):** career `retired`→tinyint(1) (#135).
...
**Table evidence (2026-07-25):** team_info booleans→tinyint(1) (#135).
...
**Table evidence (2026-07-25):** ibl_demands pid FK + PK (#1037).
...
**Table evidence (2026-07-25):** trade_info FK (migration 067).
...
**Table evidence (2026-07-25):** box_scores teamid FK (#142).
...
**Table evidence (2026-07-25):** box_scores_teams.name NOT NULL (#138).
...
**Table evidence (2026-07-25):** one_on_one winner_pid/loser_pid FKs (#139).
...
**Table evidence (2026-07-25):** ASG/EOY ballot cols→varchar(128) (#145); further shrink needs prod MAX audit (data-blocked).
...
**Table evidence (2026-07-25):** **Marker was missing/stale.** ibl_draft.team varchar(255)→varchar(35) (#1157, maintenance-41c).
...
**Table evidence (2026-07-25):** migrations/README strip stale "pending"/dead refs; docs-only. **Status:** Stripped stale "PENDING"/roadmap framing and dead doc refs from `ibl5/migrations/README.md` (this PR).
...
**Table evidence (2026-07-25):** league_config teamid FK + trigger (#140).
...
**Table evidence (2026-07-25):** box_scores.pos→ENUM (#135).
...
**Table evidence (2026-07-25):** covering index (migration 091/121).
...
**Table evidence (2026-07-25):** 113/117/125 idempotent (maintenance-27).
