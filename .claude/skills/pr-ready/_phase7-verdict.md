...
2. **Post the sticky verdict.** Marker, placed as the **last line of the body** so an update matches:
   ...
   **Then emit the machine-readable verdict marker — the last tool call of the run.** The comment is posted; this publishes the same verdict in a form `bin/pr-ready-now` can read without parsing prose. Pick the token mechanically from the READY / NOT READY line just written — no judgment: `NOT-READY` if that line says NOT READY; `READY-WITH-NOTES` if it says READY **and** any hold predicate printed something other than `(clear)` or Phase 6.5 left anything `not fixed — filed`; `READY` otherwise.
