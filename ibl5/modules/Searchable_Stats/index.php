...
function chunkstats()
{
    ...
    $max_chunk_query = "SELECT MAX(chunk) as maxchunk FROM nuke_ijblplyr_chunk WHERE active = 1";
    ...
    $query = "SELECT * FROM nuke_ijblplyr_chunk WHERE $argument AND qa !=0 ORDER BY $sort DESC";
    $result = $db->sql_query($query);
    $num = $db->sql_numrows($result);

    echo "<form name=\"Leaderboards\" method=\"post\" action=\"http://www.ijbl.net/modules.php?name=Chunk_Stats&op=chunk\">";
    echo "<table border=1>";
    echo "<tr><td><b>Chunk</td><td><select name=\"chunk\">";
    chunk_option($row[maxchunk], $chunk);
    echo "</select></td>";
    echo "<td><b>Team</td><td><select name=\"team\">";
    team_option($team);
    echo "</select></td>";
    ...
    echo "<table cellpadding=3 CELLSPACING=0 border=0><tr bgcolor=D99795><td><b>Rank</td><td><b>Name</td><td><b>Pos</td><td><b>Team</td><td><b>Chunk</td><td><b>G</td><td><b>Min</td><td allign=right><b>fgm</td><td><b>fga</td><td allign=right><b>fg%</td><td><b>ftm</td><td allign=right><b>fta</td><td><b>ft%</td><td allign=right><b>tgm</td><td><b>tga</td><td allign=right><b>tg%</td><td><b>orb</td><td allign=right><b>reb</td><td><b>ast</td><td allign=right><b>stl</td><td><b>to</td><td allign=right><b>blk</td><td><b>pf</td><td allign=right><b>ppg</td><td><b>QA</td></tr>";
...
function seasonstats()
{
    ...
    $query = "SELECT * FROM nuke_ijblplyr WHERE retired = 0 $argument ORDER BY $sort DESC";
    $result = $db->sql_query($query);
    $num = $db->sql_numrows($result);

    echo "<form name=\"Leaderboards\" method=\"post\" action=\"http://www.ijbl.net/modules.php?name=Chunk_Stats&op=season\">";
    echo "<table border=1>";
...
function team_option($team_selected)
{
    ...
    $query = "SELECT * FROM nuke_ijbl_power WHERE TeamID BETWEEN 1 AND 28 ORDER BY TeamID ASC";
    $result = $db->sql_query($query);
    $num = $db->sql_numrows($result);
    echo "<option value=0>All</option>";
    $i = 0;
