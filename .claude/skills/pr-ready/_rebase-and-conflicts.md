...
- **First returned line, before any other work:** `pwd` and `git rev-parse --show-toplevel`. A mismatch against the expected worktree means the session is not in the PR's tree — the Phase 0.4 `EnterWorktree` switch did not take, or the run started in the wrong tree and Phase 0.3 let it through — and everything after it would rebase the wrong tree.
