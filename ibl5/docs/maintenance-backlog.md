...
**Status:** Completed (maintenance-28, migration 136) — 16 ASG + 9 EOY player-name ballot columns resized `varchar(255)` → `varchar(32)` (= `ibl_plr.name`); 3 EOY GM-of-Year columns → `varchar(25)` (= `ibl_team_info.gm_username`). Safe by construction: every stored value originates from one of those source columns, so the shrink cannot truncate. Child-table normalization with an `ibl_plr.pid` FK remains the longer-term option.
