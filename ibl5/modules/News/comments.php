    ...
    if ($mode == 'nested') {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if ($r_score >= $thold) {
                ...
                $comments++;
                if (!mb_eregi("[a-z0-9]", $r_name)) {
                    $r_name = $anonymous;
                }

                if (!mb_eregi("[a-z0-9]", $r_subject)) {
                ...
                $options .= "&order=" . $order;
                $options .= "&thold=" . $thold;
                if ((isset($userinfo['commentmax'])) and (strlen($r_comment) > $userinfo['commentmax'])) {
                    echo substr("$r_comment", 0, $userinfo['commentmax']) . "<br><br><b><a href=\"modules.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">" . _READREST . "</a></b>";
                } elseif (strlen($r_comment) > $commentlimit) {
                    echo substr("$r_comment", 0, $commentlimit) . "<br><br><b><a href=\"modules.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">" . _READREST . "</a></b>";
                } else {
                    echo $r_comment;
                }
...
function DisplayTopic($sid, $pid = 0, $tid = 0, $mode = "thread", $order = 0, $thold = 0, $level = 0, $nokids = 0)
{
    ...
    while ($count_times < $num_tid) {
        ...
        $options .= "&order=" . $order;
        $options .= "&thold=" . $thold;
        if ((isset($userinfo['commentmax'])) and (strlen($comment) > $userinfo['commentmax'])) {
            echo substr($comment, 0, $userinfo['commentmax']) . "<br><br><b><a href=\"modules.php?name=$module_name&amp;file=comments&amp;sid=$r_sid&amp;tid=$r_tid$options\">" . _READREST . "</a></b>";
        } elseif (strlen($comment) > $commentlimit) {
            echo substr($comment, 0, $commentlimit) . "<br><br><b><a href=\"modules.php?name=$module_name&amp;file=comments&amp;sid=$sid&tid=$tid$options\">" . _READREST . "</a></b>";
        } else {
            echo $comment;
        }
