...
└── PlayerDatabaseView.php        - HTML rendering (output buffering)

modules/PlayerDatabase/
└── index.php                   - Controller (~70 lines)

tests/PlayerDatabase/
├── PlayerDatabaseValidatorTest.php  - 20 tests
├── PlayerDatabaseRepositoryTest.php - 9 tests
├── PlayerDatabaseServiceTest.php    - 7 tests
└── PlayerDatabaseViewTest.php       - 18 tests
```

## Key Features

### Validator (`PlayerDatabaseValidator`)
...
```php
// Initialize classes
$validator = new \PlayerDatabase\PlayerDatabaseValidator();
...
Run tests:
```bash
cd ibl5 && vendor/bin/phpunit tests/PlayerDatabase/
```
