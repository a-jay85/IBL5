...
| [0108](0108-bug-pipeline-test-isolation.md) | The bug-pipeline test-isolation boundary | Accepted | Structural isolation via three orthogonal barriers (separate Discord app, scratch DB `ibl5_bug_pipeline_test`, stubbed `GH_BIN`); all asserted before the first write by `bin/bug-pipeline-e2e`. |
