...
5. **Re-watch CI on the new head — exactly one delegate.** The Phase 4.5 delegate has already returned (that is what re-invoked you), so there is nothing to stop. Run `git rev-parse HEAD` bare and record the new `<HEAD_SHA>` literal.
...
6. **One bounded staleness check, then stop.** After `CI COMPLETE`, and only when `<STRICT>` is `true`, read once:
