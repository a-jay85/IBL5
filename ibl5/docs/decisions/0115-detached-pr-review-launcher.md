...
- **Share `bin/pr-ready-now` (example)'s slot pool** — reuse one accounting surface for both launchers. Rejected because: a `/pr-ready` run holding its own slot would be counted against the review it just triggered, and the two workloads have different lifetimes. <!-- slop-ok -->
...
- `bin/post-plan-fleet` — the mirrored architecture (renamed from `bin/pr-ready-now` (example) when /pr-ready was retired). <!-- slop-ok -->
- `.claude/skills/pr-ready/scripts/review-owed.sh` (example) — the fire path, including the live-slot skip. (script deleted when /pr-ready was retired) <!-- slop-ok -->
- `.claude/review-shared/_phase7-verdict.md` — the terminator amendment naming this as the one permitted action. <!-- slop-ok -->
