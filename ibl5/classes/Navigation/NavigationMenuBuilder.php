...
class NavigationMenuBuilder implements NavigationMenuBuilderInterface
{
    ...
    /**
     * Check if waivers are allowed based on season phase and toggle.
     *
     * Mirrors Season::areWaiversAllowed() logic using NavigationConfig fields:
     * - HEAT/Regular Season/Playoffs: always allowed
     * - Draft: never allowed
     * - Free Agency/Preseason: depends on toggle
     */
    private function areWaiversAllowed(): bool
    {
        $phase = $this->config->seasonPhase;

        if (in_array($phase, ['HEAT', 'Regular Season', 'Playoffs'], true)) {
            return true;
        }
        ...
        return $this->config->allowWaivers === \Settings\SettingName::AllowWaiverMoves->enabledValue();
    }
