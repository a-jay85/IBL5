...
if (is_user($user)) {
    ...
    if (isset($title)) {$title = filter($title, "nohtml");
        $title = addslashes($title);} else { $title = "";}
    if (isset($jbodytext)) {$jbodytext = kses(ADVT_stripslashes($jbodytext), $allowed);
        $jbodytext = addslashes($jbodytext);} else { $jbodytext = "";}
    if (isset($mood)) {$mood = filter($mood, "nohtml");} else { $mood = "";}
    if (isset($status)) {$status = filter($status, "nohtml");} else { $status = "";}
    if (isset($jid)) {$jid = intval($jid);} else { $jid = "";}
