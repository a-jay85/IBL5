...
- Amendment (2026-09-17, on the disposition hold added after this ADR): the `--queue` default becomes unconditional. Two later commits (a525358b3 in PR #1757, 71918fdf0 in PR #2040) added a second disposition rule to `/plan` Step 5.5 and to `/plan-prompt` Step 5: work tripping a `plan-architect-xhigh` trigger was held back from the queue so a human would read the plan first. That rule is deleted from both skills. The trigger list keeps the job it was written for, which is selecting the architect tier. Holding the plan bought nothing the merge path does not already buy: `/post-plan` always opens the PR, Phase 6.5's arming conditions only ever *hold*, `auto_merge: false` still holds any plan whose realized diff wants a signoff, and a `feat:` title stays held by the commit-type floor and the `human-signoff` required check. Against that, a held plan decays on disk the same way the unpasted prompt this ADR exists to eliminate decays. `--implement` survives with one meaning: the user asked, in that conversation, to read the plan before it implements. A drafting session never picks it on the user's behalf, and intent drift surfaces at the PR.
...
- `.claude/skills/plan/SKILL.md`: Step 5.5, the disposition precedence the 2026-09-17 amendment shortens to two rules.
