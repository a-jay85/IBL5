...
`a-jay85/IBL5-backlog` is the **single source of truth** for J-series work: the OPEN list, each item's current state, AND its "Do NOT re-open / NOT-A-LEVER" trap list all live **in the backlog J-entry**, self-standing with each item's proof. There is no companion memory to keep in sync — that split-brain (a hand-maintained frontier hash ledger) was **eliminated 2026-07-21** because it drifted against git and against the backlog. Do not re-create it.
...
Do NOT backfill hashes into the live backlog — that just relocates the cache you're eliminating. If a one-time provenance snapshot is ever wanted, it belongs in `a-jay85/IBL5-backlog` (history, never maintained), not the live doc.
...
shifted the four line anchors in `jsb-j7-param-recovery.md` by between 18 and 38 lines each. The
