...
deliberately.
**Narrowed by:** [ADR-0104](0104-verification-only-doc-refresh-auto-merge.md). Decision 2's human-merge posture still governs, except for verification-only diffs (date bumps and same-line numeric corrections on tracked files under `ibl5/docs/` or at `ibl5/classes/*/README.md`, per ADR-0104's 2026-09-24 addendum) proven by `bin/docfix-check-veronly`.
