...
function showpage($playerID, $pageView)
{
    ...
    if ($pageView == PlayerPageType::OLYMPIC_TOTALS) {
        ...
        $resultplayoff4 = $db->sql_query("SELECT * FROM ibl_olympics_stats WHERE name='$player->name' ORDER BY year ASC");
        while ($rowplayoff4 = $db->sql_fetchrow($resultplayoff4)) {
            $hist_year = stripslashes(check_html($rowplayoff4['year'], "nohtml"));
            $hist_team = stripslashes(check_html($rowplayoff4['team'], "nohtml"));
            $hist_gm = stripslashes(check_html($rowplayoff4['games'], "nohtml"));
    ...
    if ($pageView == PlayerPageType::OLYMPIC_AVERAGES) {
        ...
        echo "<table border=1 cellspacing=0 class=\"sortable\>
            <tr>
                <td colspan=15><center><b><font class=\"content\">Olympics Career Averages</font></b></center></td>
            </tr>
            <tr>
                <td>year</td>
                <td>team</td>
        ...
        echo "<tr><td colspan=2>Olympics Averages</td>
            <td><center>$car_gm</center></td>
            <td><center>";
        printf('%01.1f', $car_avgm);
        echo "</center></td><td><center>";
