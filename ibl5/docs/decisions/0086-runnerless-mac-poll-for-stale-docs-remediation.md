...
## Amendment — 2026-08-08: autonomous reaper, fail-closed probes, prompt delivery
...
**Prompt-file delivery (defect 4)** — `bin/docfix-run` previously embedded `$PROMPT` directly into the
plist's `/bin/bash -lc "…"` string via an unquoted heredoc. This had two consequences: (a) backticks in
the prompt text (`\`bin/check-docs\``, `\`bin/post-plan-now\``) were executed by the plist's bash at
runtime; (b) `$SIG` (built from repo file paths in the workflow) was an injection path. The fix: write
`$PROMPT` to `/tmp/docfix-prompt-$SLUG-$TS.txt` via `printf '%s\n'` (no re-evaluation), then read it
back in the plist via `"\$(cat "/tmp/...")"`  — a quoted command substitution whose output bash does not
re-scan. Prompt text must never again be interpolated into the plist XML.

**Invariant:** `auto_merge: false` on all plans generated by `bin/docfix-run`. The reaper and its
