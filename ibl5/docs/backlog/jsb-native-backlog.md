...
J23 round-half-up + base_time re-center (✅, 2026-07-16, #1495) — coupled faithful fix deferred from J21; ADR-0085 records the hold finding; shipped round-half-up + baseTimeMid re-center 14.5→13.65 (J23)
  └─→ J24 possession-clock subsystem port (◑ Partial 2026-07-17) — step classes + jitter SHIPPED (u=0: base_time constant, ratio dead code); Phase 5 NO-GO: engine arms fast classes ~29% vs real ~11.5%, Var(lnPOSS) unmoved (0.000270 vs 0.000721), Cov not flipped; provisional center re-centered 13.65→17.7; residuals: arming-share gap, CEngine+0x30 flag, .lge +0x12c strategy_adj
