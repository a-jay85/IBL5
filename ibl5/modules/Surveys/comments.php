        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if ($r_score >= $thold) {
                ...
                if ((isset($userinfo['commentmax'])) && (strlen($r_comment) > $userinfo['commentmax'])) {
                    echo substr($r_comment, 0, $userinfo['commentmax']) . "<br><br><b><a href=\"modules.php?name=$module_name&file=comments&pollID=$r_pollID&tid=$r_tid&mode=$mode&order=$order&thold=$thold\">" . _READREST . "</a></b>";
                } elseif (strlen($r_comment) > $commentlimit) {
                    echo substr("$r_comment", 0, $commentlimit) . "<br><br><b><a href=\"modules.php?name=$module_name&amp;file=comments&amp;pollID=$r_pollID&amp;tid=$r_tid&amp;mode=$mode&amp;order=$order&amp;thold=$thold\">" . _READREST . "</a></b>";
                } else {
                    echo $r_comment;
                }
