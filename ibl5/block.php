 ...
 *
 * @phpstan-scan-pending this file is not yet in phpstan parameters.paths (see plan [[11.2]])
 */
...
foreach ($autoRejections as $rejection) {
    ...
    $autoRejectedText .= "\n" . HtmlSanitizer::e($rejection['teamName']) . "'s offer for " . HtmlSanitizer::e($rejection['playerName']) . ": ";
...
<head>
    ...
    <link rel="stylesheet" href="design/components/block-fa-admin.css">
</head>
<body>
    ...
    <?php if ($actionMessage !== ''): ?>
        <p id="actionMessage" class="<?= $actionCompleted ? 'message-success' : 'message-error' ?>">
            <?= HtmlSanitizer::e($actionMessage) ?>
        </p>
    <?php endif; ?>

    <table>
        <tr class="block-fa-row-bold">
        ...
        <tr>
            <td><?= HtmlSanitizer::e($signing['playerName']) ?></td>
            <td><?= HtmlSanitizer::e($signing['teamName']) ?></td>
    ...
    <table>
        ...
        <tr>
            <td><?= HtmlSanitizer::e($offer['playerName']) ?></td>
            <td><?= HtmlSanitizer::e($offer['teamName']) ?></td>
            ...
            <td><?= \BasketballStats\StatsFormatter::formatWithDecimals($offer['perceivedValue'], 2) ?></td>
        </tr>
        <?php endforeach; ?>
    </table>
    ...
    <?php if ($day < 12): ?>
    <h2 class="block-fa-reject-header">AUTO-REJECTED OFFERS</h2>
    <textarea cols="85" rows="20"><?= HtmlSanitizer::e($autoRejectedText) ?></textarea>
    <hr>
    <?php endif; ?>

    <h2 class="block-fa-discord-header">ALL REMAINING OFFERS IN DISCORD FORMAT (FOR <a href="https://discord.com/channels/666986450889474053/682990441641279531">#live-sims</a>)</h2>
    <textarea class="block-fa-discord-textarea" cols="85" rows="20"><?= HtmlSanitizer::e($discordText) ?></textarea>
    <hr>
    ...
    <textarea id="signingsDisplay" cols="125" rows="10" readonly><?php
        foreach ($signings as $signing) {
            echo HtmlSanitizer::e($signing['playerName']) . ' -> ' . HtmlSanitizer::e($signing['teamName']);
    ...
    <textarea id="newsHometextArea" cols="125" rows="20"><?= HtmlSanitizer::e($newsHomeText) ?></textarea>

    <hr>
    <h2>ALL OFFERS IN HTML FORMAT (FOR NEWS ARTICLE EXTENDED TEXT)</h2>
    <textarea id="newsBodytextArea" cols="125" rows="20"><?= HtmlSanitizer::e($newsBodyText) ?></textarea>
        ...
        <div class="modal-content">
            ...
            <div class="modal-buttons">
                <form method="POST" id="assignFreeAgentsForm">
                    <input type="hidden" name="_csrf_token" value="<?= HtmlSanitizer::e($csrfToken) ?>">
                    <input type="hidden" name="action" value="assign_free_agents">
                    <input type="hidden" name="signings_data" id="signingsDataInput" value="<?= HtmlSanitizer::e($signingsDataJson) ?>">
