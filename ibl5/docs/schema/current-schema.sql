...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_season1993_phantom_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_season1993_phantom_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) DEFAULT '' COMMENT 'Player name (denormalized snapshot)',
  `pos` enum('PG','SG','SF','PF','C','G','F','GF','') DEFAULT '' COMMENT 'Player position at game time',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_season2004_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_season2004_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) DEFAULT '' COMMENT 'Player name (denormalized snapshot)',
  `pos` enum('PG','SG','SF','PF','C','G','F','GF','') DEFAULT '' COMMENT 'Player position at game time',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_season2007_phantom_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_season2007_phantom_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) DEFAULT '' COMMENT 'Player name (denormalized snapshot)',
  `pos` enum('PG','SG','SF','PF','C','G','F','GF','') DEFAULT '' COMMENT 'Player position at game time',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_teams_season1993_phantom_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_teams_season1993_phantom_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) NOT NULL DEFAULT '' COMMENT 'Denormalized team label snapshot (e.g. Team Home/Team Away); no FK by design',
  `game_of_that_day` int(11) DEFAULT NULL COMMENT '1-based index of this game within its date across the whole league (NOT a doubleheader / per-matchup counter)',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_teams_season2004_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_teams_season2004_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) NOT NULL DEFAULT '' COMMENT 'Denormalized team label snapshot (e.g. Team Home/Team Away); no FK by design',
  `game_of_that_day` int(11) DEFAULT NULL COMMENT '1-based index of this game within its date across the whole league (NOT a doubleheader / per-matchup counter)',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_box_scores_teams_season2007_phantom_backup`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_box_scores_teams_season2007_phantom_backup` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `game_date` date NOT NULL COMMENT 'Game date',
  `name` varchar(16) NOT NULL DEFAULT '' COMMENT 'Denormalized team label snapshot (e.g. Team Home/Team Away); no FK by design',
  `game_of_that_day` int(11) DEFAULT NULL COMMENT '1-based index of this game within its date across the whole league (NOT a doubleheader / per-matchup counter)',
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_events`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_events` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `request_uri` varchar(512) NOT NULL COMMENT 'Raw REQUEST_URI, truncated to 512 chars',
  `route_name` varchar(64) DEFAULT NULL COMMENT 'Module route from ?name= (raw, sanitized+truncated in PHP); NULL for non-module pages',
  `http_method` varchar(10) NOT NULL DEFAULT '' COMMENT 'REQUEST_METHOD (GET/POST/...)',
  `username` varchar(60) DEFAULT NULL COMMENT 'Authenticated username; NULL when anonymous',
...
CREATE TABLE `ibl_olympics_plr_snapshots` (
  ...
  `po_phantom_games` tinyint(3) unsigned NOT NULL DEFAULT 0,
...
/*!40101 SET character_set_client = @saved_cs_client */;
DROP TABLE IF EXISTS `ibl_team_awards_conference_backup_165`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `ibl_team_awards_conference_backup_165` (
  `year` smallint(5) unsigned NOT NULL DEFAULT 0 COMMENT 'Season year of award',
  `name` varchar(35) NOT NULL COMMENT 'Team name',
  `award` varchar(350) NOT NULL COMMENT 'Award description',
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `ibl_season_career_avgs` AS select `bs`.`pid` AS `pid`,`p`.`name` AS `name`,cast(count(0) as signed) AS `games`,round(avg(`bs`.`game_min`),2) AS `minutes`,round(avg(`bs`.`calc_fg_made`),2) AS `fgm`,round(avg(`bs`.`game_2ga` + `bs`.`game_3ga`),2) AS `fga`,case when sum(`bs`.`game_2ga` + `bs`.`game_3ga`) > 0 then round(sum(`bs`.`calc_fg_made`) / sum(`bs`.`game_2ga` + `bs`.`game_3ga`),3) else 0.000 end AS `fgpct`,round(avg(`bs`.`game_ftm`),2) AS `ftm`,round(avg(`bs`.`game_fta`),2) AS `fta`,case when sum(`bs`.`game_fta`) > 0 then round(sum(`bs`.`game_ftm`) / sum(`bs`.`game_fta`),3) else 0.000 end AS `ftpct`,round(avg(`bs`.`game_3gm`),2) AS `tgm`,round(avg(`bs`.`game_3ga`),2) AS `tga`,case when sum(`bs`.`game_3ga`) > 0 then round(sum(`bs`.`game_3gm`) / sum(`bs`.`game_3ga`),3) else 0.000 end AS `tpct`,round(avg(`bs`.`game_orb`),2) AS `orb`,round(avg(`bs`.`game_drb`),2) AS `drb`,round(avg(`bs`.`calc_rebounds`),2) AS `reb`,round(avg(`bs`.`game_ast`),2) AS `ast`,round(avg(`bs`.`game_stl`),2) AS `stl`,round(avg(`bs`.`game_tov`),2) AS `tvr`,round(avg(`bs`.`game_blk`),2) AS `blk`,round(avg(`bs`.`game_pf`),2) AS `pf`,round(avg(`bs`.`calc_points`),2) AS `pts`,`p`.`retired` AS `retired` from (`ibl_box_scores` `bs` join `ibl_plr` `p` on(`bs`.`pid` = `p`.`pid`)) where `bs`.`game_type` = 1 and `bs`.`game_min` > 0 and `bs`.`teamid` not in (40,41,50,51) group by `bs`.`pid`,`p`.`name`,`p`.`retired` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
/*!50001 DROP VIEW IF EXISTS `ibl_sophomore_career_totals`*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `ibl_team_win_loss` AS with canonical_games as (select `ibl_box_scores_teams`.`game_date` AS `game_date`,`ibl_box_scores_teams`.`visitor_teamid` AS `visitor_teamid`,`ibl_box_scores_teams`.`home_teamid` AS `home_teamid`,min(`ibl_box_scores_teams`.`game_of_that_day`) AS `game_of_that_day` from `ibl_box_scores_teams` where `ibl_box_scores_teams`.`game_type` = 1 group by `ibl_box_scores_teams`.`game_date`,`ibl_box_scores_teams`.`visitor_teamid`,`ibl_box_scores_teams`.`home_teamid`), unique_games as (select `b`.`game_date` AS `game_date`,`b`.`visitor_teamid` AS `visitor_teamid`,`b`.`home_teamid` AS `home_teamid`,`b`.`visitor_q1_points` + `b`.`visitor_q2_points` + `b`.`visitor_q3_points` + `b`.`visitor_q4_points` + coalesce(`b`.`visitor_ot_points`,0) AS `visitor_total`,`b`.`home_q1_points` + `b`.`home_q2_points` + `b`.`home_q3_points` + `b`.`home_q4_points` + coalesce(`b`.`home_ot_points`,0) AS `home_total` from (`ibl_box_scores_teams` `b` join `canonical_games` `c` on(`c`.`game_date` = `b`.`game_date` and `c`.`visitor_teamid` = `b`.`visitor_teamid` and `c`.`home_teamid` = `b`.`home_teamid` and `c`.`game_of_that_day` <=> `b`.`game_of_that_day`)) where `b`.`game_type` = 1 group by `b`.`game_date`,`b`.`visitor_teamid`,`b`.`home_teamid`), team_games as (select `unique_games`.`visitor_teamid` AS `teamid`,`unique_games`.`game_date` AS `game_date`,if(`unique_games`.`visitor_total` > `unique_games`.`home_total`,1,0) AS `win`,if(`unique_games`.`visitor_total` < `unique_games`.`home_total`,1,0) AS `loss` from `unique_games` union all select `unique_games`.`home_teamid` AS `teamid`,`unique_games`.`game_date` AS `game_date`,if(`unique_games`.`home_total` > `unique_games`.`visitor_total`,1,0) AS `win`,if(`unique_games`.`home_total` < `unique_games`.`visitor_total`,1,0) AS `loss` from `unique_games`)select case when month(`tg`.`game_date`) >= 10 then year(`tg`.`game_date`) + 1 else year(`tg`.`game_date`) end AS `year`,`ti`.`team_name` AS `currentname`,coalesce(`fs`.`team_name`,`ti`.`team_name`) AS `namethatyear`,cast(sum(`tg`.`win`) as unsigned) AS `wins`,cast(sum(`tg`.`loss`) as unsigned) AS `losses` from ((`team_games` `tg` join `ibl_team_info` `ti` on(`ti`.`teamid` = `tg`.`teamid`)) left join `ibl_franchise_seasons` `fs` on(`fs`.`franchise_id` = `tg`.`teamid` and `fs`.`season_ending_year` = case when month(`tg`.`game_date`) >= 10 then year(`tg`.`game_date`) + 1 else year(`tg`.`game_date`) end)) group by `tg`.`teamid`,case when month(`tg`.`game_date`) >= 10 then year(`tg`.`game_date`) + 1 else year(`tg`.`game_date`) end,`ti`.`team_name`,coalesce(`fs`.`team_name`,`ti`.`team_name`) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
/*!50001 DROP VIEW IF EXISTS `vw_career_totals`*/;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013  SQL SECURITY DEFINER */
/*!50001 VIEW `vw_team_awards` AS select `ibl_team_awards`.`year` AS `year`,`ibl_team_awards`.`name` AS `name`,`ibl_team_awards`.`award` AS `award`,`ibl_team_awards`.`id` AS `id` from `ibl_team_awards` union all select `ranked`.`year` AS `year`,`ranked`.`name` AS `name`,'IBL Champions' AS `award`,0 AS `id` from (select `psr`.`year` AS `year`,`psr`.`winner` AS `name`,`psr`.`round` AS `round`,max(`psr`.`round`) over ( partition by `psr`.`year`) AS `max_round`,count(0) over ( partition by `psr`.`year`,`psr`.`round`) AS `series_in_round` from `vw_playoff_series_results` `psr`) `ranked` where `ranked`.`round` = `ranked`.`max_round` and `ranked`.`series_in_round` = 1 union all select `ranked`.`year` AS `year`,`ranked`.`name` AS `name`,`ranked`.`award` AS `award`,0 AS `id` from (select `psr`.`year` AS `year`,`psr`.`winner` AS `name`,concat(`lc`.`conference`,' Conference Champions') AS `award`,count(0) over ( partition by `psr`.`year`,`lc`.`conference`) AS `series_in_conf` from ((`vw_playoff_series_results` `psr` left join `ibl_franchise_seasons` `fs` on(`fs`.`franchise_id` = `psr`.`winner_tid` and `fs`.`season_ending_year` = `psr`.`year`)) join `ibl_league_config` `lc` on(`lc`.`season_ending_year` = `psr`.`year` and `lc`.`team_name` = coalesce(`fs`.`team_name`,`psr`.`winner`))) where `psr`.`round` = 3 and `psr`.`winner_games` >= nullif(cast(substring_index(`lc`.`playoff_round3_format`,' ',1) as unsigned),0)) `ranked` where `ranked`.`series_in_conf` = 1 union all select `hc`.`year` AS `year`,`ti`.`team_name` AS `name`,'IBL HEAT Champions' AS `award`,0 AS `id` from ((select year(`bst`.`game_date`) AS `year`,case when `bst`.`home_q1_points` + `bst`.`home_q2_points` + `bst`.`home_q3_points` + `bst`.`home_q4_points` + coalesce(`bst`.`home_ot_points`,0) > `bst`.`visitor_q1_points` + `bst`.`visitor_q2_points` + `bst`.`visitor_q3_points` + `bst`.`visitor_q4_points` + coalesce(`bst`.`visitor_ot_points`,0) then `bst`.`home_teamid` else `bst`.`visitor_teamid` end AS `winner_tid`,row_number() over ( partition by year(`bst`.`game_date`) order by `bst`.`game_date` desc,`bst`.`game_of_that_day`) AS `rn` from `ibl_box_scores_teams` `bst` where `bst`.`game_type` = 3) `hc` join `ibl_team_info` `ti` on(`ti`.`teamid` = `hc`.`winner_tid`)) where `hc`.`rn` = 1 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
/*!50001 DROP VIEW IF EXISTS `vw_team_standings`*/;
