...
function displayNavigationMenu($currentSeasonType)
{
    $menuItems = array(
        'combined' => 'All Games Combined',
        'regular' => 'Regular Season',
        'playoffs' => 'Playoffs',
        'heat' => 'HEAT'
    );

    echo "<center><div style='margin: 20px 0; padding: 10px; border: 2px solid #333; background-color: #f0f0f0;'>";
    echo "<b>Select Season Type:</b> &nbsp;&nbsp;";
    
    $first = true;
    foreach ($menuItems as $type => $label) {
        if (!$first) {
            echo " | ";
        }
        if ($type == $currentSeasonType) {
            echo "<b><u>$label</u></b>";
        } else {
            echo "<a href=\"modules.php?name=All_Time_Win_Loss_Records&seasonType=$type\">$label</a>";
        }
        $first = false;
    }
...
function queryAllTimeWinLossRecords($seasonType)
{
    global $db;

    // Build WHERE clause based on season type
    $whereClause = "";
    switch ($seasonType) {
        case 'regular':
            // Regular season: November through May (months 11, 12, 1-5)
            $whereClause = "WHERE MONTH(Date) IN (11, 12, 1, 2, 3, 4, 5)";
            break;
        case 'playoffs':
            // Playoffs: June (month 6)
            $whereClause = "WHERE MONTH(Date) = 6";
            break;
        case 'heat':
            // HEAT: October (month 10)
            $whereClause = "WHERE MONTH(Date) = 10";
            break;
        case 'combined':
        default:
            // All games
    ...
    $query = "SELECT self, opponent, SUM(wins) AS wins, SUM(losses) AS losses
				FROM (
					...
					FROM ibl_schedule
					$whereClause
					" . ($whereClause ? "AND" : "WHERE") . " HScore > VScore
					...
					FROM ibl_schedule
					$whereClause
					" . ($whereClause ? "AND" : "WHERE") . " VScore > HScore
					GROUP BY self, opponent
...
function displayAllTimeWinLossRecords($tid, $seasonType)
{
    global $db;
    ...
    switch ($seasonType) {
        case 'regular':
            ...
            break;
        case 'playoffs':
            $seasonTypeLabel = 'Playoffs';
            break;
        case 'heat':
            $seasonTypeLabel = 'HEAT';
            break;
        case 'combined':
        default:
            $seasonTypeLabel = 'All Games Combined';
            break;
    }

    echo "<center><h2>All-Time Win/Loss Records - $seasonTypeLabel</h2></center>";
    ...
    $resultWinLossRecords = queryAllTimeWinLossRecords($seasonType);
    ...
    while ($tidRow <= $numteams) {
        ...
        while ($tidColumn <= $numteams) {
            if ($tidRow == $tidColumn) {
                ...
                $row = $db->sql_fetch_assoc($resultWinLossRecords);
                if ($row['self'] == $tidRow and $row['opponent'] == $tidColumn) {
                    ...
                    mysqli_data_seek($resultWinLossRecords, $pointer); // Bring the pointer back since no record was found
                }
            }
            $tidColumn++;
        }
