...
**Persist before returning (timeout durability).** The `plan-architect` agent has no `Write`/`Edit` tool, so its only delivery channel is its streamed final message — and a long final message can be lost to an `API Error: Stream idle timeout`, which is **unrecoverable** (this harness has no resume / `SendMessage`, and the agent cannot self-persist a draft once the message drops). Defend against this in the prompt: instruct the agent that the **moment its plan markdown is composed** — not "at the end," not "before the final message," but the instant the full plan text exists in its head — it MUST write that complete markdown to the **Draft output path** (item 5) via Bash, then return only a **one-line pointer** (`Plan written to <draft path>`) — never the plan body. The framing matters: an agent told "persist before your final message" tends to keep working (one more grep, one more refinement) right up to the message and only then persist, so the stall still catches it with nothing on disk. Tell it instead that the Bash-persist is the *first* thing it does once the plan is fully formed, and that any tool call it makes after that point is a refinement on an already-durable artifact, not a prerequisite to saving one. Give it this exact recipe:
