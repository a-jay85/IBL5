...
func countStealTurnovers(victimTVR, defenderSTL int, seeds uint64) int {
	...
	for s := uint64(1); s <= seeds; s++ {
		...
		gs := &gameState{rng: rng.New(s), period: 1, clock: 500, stealTurnoverScale: stealTurnoverScale, nonStealTurnoverScale: nonStealTurnoverScale, foulBucketScale: foulBucketScale, andOneMadeRateScale: andOneMadeRateScale}
...
func TestTurnoverProb_NoNaNOnEmptyRatings(t *testing.T) {
	...
	gs := &gameState{stealTurnoverScale: stealTurnoverScale, nonStealTurnoverScale: nonStealTurnoverScale, foulBucketScale: foulBucketScale, andOneMadeRateScale: andOneMadeRateScale}
