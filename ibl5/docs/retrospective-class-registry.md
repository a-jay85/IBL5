...
| 2026-08-27 | #2002 | class: Phase 6 conflict-audit runtime dependency (/tmp/pr-ready-diff-pre-<N>.patch) deletable by bin/pr-ready-now:246 (rm -f /tmp/pr-ready-*-"${PR}".*), leaving the audit unable to verify conflict resolution without a reconstruction workaround | routed to: Rung 2 — correction (2026-09-16): the Rung 3 forced-trigger row this row originally named was never written, and is absent from the entire history of .claude/review-shared/_plan-verification.md. The remedy that shipped is not the one proposed: the glob still matches the audit path, but the sweep is liveness-gated so only a provably dead label's scratch is cleared (per bin/pr-ready-now:597-615). Regression-tested by bin/test-pr-ready-now case 34 — dead-label clear (:919), PR-anchored glob (:930-931), live-run scratch preserved (:938-939) | prior: -- |
...
| 2026-08-31 | #2039 | class: an awk filter in a skill file uses a reset pattern that fires after the set pattern on the same diff-header line, making the exclusion a no-op and silently passing all lines through | routed to: Rung 3 - new forced-trigger row in .claude/review-shared/_plan-verification.md (section: Forced integration-verification trigger, anchor: "adds or modifies an awk filter in a skill or bin/ file"): any plan adding or modifying an awk filter in a skill or bin/ file must carry a CLI-executable smoke test that verifies the negative path (excluded content absent from output) and the positive path (non-excluded content present) | prior: -- |
...
| 2026-08-31 | #2040 | class: a backlog entry archived in the live file receives a table-row status flip but its section-body Status: line is left at the old open status — the two representations diverge until a follow-up fix commit corrects the section body | routed to: retired, class unreachable — correction (2026-09-16): the Rung 3 forced-trigger row this row named was never written, and is absent from the entire history of .claude/review-shared/_plan-verification.md. The in-repo backlog corpus it governed was deleted by PR #2182 (ADR-0121 Phase 2); ibl5/docs/backlog/ no longer exists, so no plan can trigger this class. Re-register if an in-repo backlog is reintroduced | prior: -- |
| 2026-09-04 | #2092 | class: a plan-level portability claim for a shell script uses find -regex with \{n\} interval notation, verified only on macOS BSD find (where BRE supports \{n\}), not on Ubuntu GNU find (which uses emacs regex type by default and does not treat \{n\} as an interval) — the regex silently matches nothing in CI, causing the script to find no directories and skip its entire body without error | routed to: Rung 3 - new forced-trigger row in .claude/review-shared/_plan-verification.md (section: Forced integration-verification trigger, anchor: "introduces a find -regex pattern claiming"): any plan introducing a find -regex pattern claiming cross-platform portability between macOS and Ubuntu must carry a CI-run verification row demonstrating the regex matches on the Ubuntu runner, OR must use bash-level character-class and length filtering instead of find interval expressions | prior: -- |
...
| 2026-09-14 | #2212 | class: a plan that changes the HTTP response shape of a PHP endpoint (inline render → PRG redirect) does not require auditing existing E2E assertions that match against the old response body shape, leaving them silently stale | routed to: Rung 3 - new forced-trigger row in .claude/review-shared/_plan-verification.md (section: Forced E2E triggers): any plan that changes a PHP page's HTTP response shape (inline → redirect, inline → JSON, response branch removed or replaced) must audit all existing E2E tests asserting against that endpoint's response body and update any assertions whose expected value depended on the old shape | prior: -- |
| 2026-09-14 | #2215 | class: a queue-lifecycle operation (dispose) uses mv to move a file, inheriting the source file's mtime rather than stamping the operation time, causing subsequent time-windowed find -newermt queries to miss the entry | routed to: Rung 2 (recurrence of #1925 Rung 3; no natural bin/check-* host for shell mv-without-touch pattern — dispose-path regression test d7 in bin/test-automouse-postplan-disposition ships in this PR as practical escalation) | prior: #1925 |
| 2026-09-14 | #2220 | class: a shell existence probe uses || echo <default> to convert command failure into a benign "not found" value, silently enabling a destructive action (DROP TABLE) on a protected resource that is temporarily unreachable — the same data-loss path the surrounding code exists to close | routed to: Rung 4 - new rule doc .claude/rules/shell-fail-open-probe.md: when a shell probe's exit status determines whether a destructive action fires, never use || echo <default> to swallow failure; use an if ! / explicit exit block so an unreachable resource aborts rather than proceeds | prior: -- |
| 2026-09-15 | #2231 | class: a plan prescribes verbatim regression test implementations for a behavior gate; the prescribed token paths resolve on disk or carry pre-existing cue words that match the old guard alternation, making the new test cases vacuous against the pre-fix state and providing no coverage of the new behavior | routed to: Rung 3 - forced-trigger row in .claude/review-shared/_plan-verification.md (Forced verification trigger): when a plan phase prescribes verbatim regression test implementations claimed to fail against the pre-fix guard, require empirical proof — run the prescribed cases against origin/master's guard and confirm the new cases exit 1 before verifying exit 0 against the patched guard | prior: -- |
| 2026-09-16 | #2232 | class: a write function accepts externally-supplied IDs and appends state records without first verifying the IDs reference real entries in the same store, silently persisting orphaned records until the next compaction | routed to: Rung 3 - new forced-trigger row in .claude/review-shared/_plan-verification.md (section: Forced integration-verification trigger): any plan adding or modifying a function that writes records keyed to externally-supplied IDs must assert that unknown IDs are ignored and produce no write to the backing store | prior: -- |
| 2026-09-16 | #2244 | class: a PR body Scope bullet carries a specific count of files or rules modified that overcounts the realized diff, because the count was written at plan-time before implementation finalized and was never re-verified against git diff --numstat before the body was committed | routed to: Rung 4 - addendum added to .claude/skills/post-plan/_pr-body-claims.md and .claude/skills/pr-ready/_phase65-remediation.md step 4 requiring a Read of _pr-body-claims.md before finalizing the body, making the count-re-check explicit in the write path; caught by Phase 5.5 fidelity reviewer as a 6d.4 finding | prior: -- |
```
...
| # | File:line | Same class? | Live? | Status |
|---|-----------|-------------|-------|--------|
| 1 | `.claude/skills/pr-ready/_rebase-and-conflicts.md` (example). Phase 2 delegate squash rule fires unconditionally regardless of `auto_merge` plan flag | yes (Check 2) | yes | fixed this pass (restored pre-squash stack; 11 commits rebased `--onto` new master; V-2c/V-4a verified) |
