...
/**
 * Extracts pure data-preparation responsibilities from TeamService.
 *
 * All methods return typed-array shapes (no HTML). The results are consumed by
 * TeamCardRenderer (view instantiation) and by TeamService (orchestration only).
 *
...
class TeamPageDataPreparer
{
    ...
    public function __construct(
        ...
        TeamQueryRepositoryInterface $teamQueryRepository,
        League $league,
    ) {
        $this->db = $db;
    ...
    /**
     * Prepare banner data (championships, conference titles, division titles).
     *
     * @return BannerData
     */
    public function prepareBannerData(Team $team): array
    {
    ...
    /**
     * Prepare current season standings and team info.
     *
     * @return CurrentSeasonData|null
     */
    public function prepareCurrentSeasonData(Team $team): ?array
    {
    ...
    /**
     * Prepare playoff series results grouped by round.
     *
     * @return PlayoffData
     */
    public function preparePlayoffData(Team $team): array
    {
        $teamName = $team->name;
    ...
    /**
     * Prepare win/loss history for regular season or HEAT.
     *
     * @param string $type 'regular' or 'heat'
     * @return WinLossHistoryData
     */
    public function prepareWinLossHistoryData(Team $team, string $type): array
    {
        if ($type === 'heat') {
    ...
    /**
     * Prepare draft picks owned by the team.
     *
     * @return list<DraftPickItemData>
     */
    public function prepareDraftPicksData(Team $team): array
    {
    ...
    /**
     * Fetch raw awards data (GM tenures, GM awards, team accomplishments).
     *
     * @return AwardsData
     */
    public function prepareAwardsData(Team $team): array
    {
        return [
