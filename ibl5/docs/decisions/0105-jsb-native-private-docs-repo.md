...
`.git/info/exclude`, so edits land on disk with no diff or attribution, and no way to
...
`~/claude-plans/jsb-j7-param-recovery.md` by between 18 and 38 lines each. An invisible edit surface
...
A blacklist false negative means data **is** committed, which is irreversible once pushed. A
whitelist false negative means data is **not** committed, which is recoverable with one more `!`
...
Because `jumpshot.exe` is untracked, CI cannot verify it. A committed byte fixture
...
**Self-contained checker.** This is a new checker rather than an extension of `bin/check-docs`. `bin/check-docs` has
hardcoded IBL5-specific `IN_SCOPE_GLOBS` and no `--root` flag. A genuine root override
would also have to make the globs configurable. That is a rewrite of its scoping model,
far beyond a minimal extension. The `meta-tooling-bar` "no host to extend" condition
applies. The cost
...
- **Track `jumpshot.exe`** so CI can run the anchor canary against it directly. Rejected:
  ...
  on a third party's infrastructure. That is not a legal posture. The property the canary
...
- **Backfill frontmatter into all 52 whitelisted `.md` files** so the freshness job can
  ...
  rolling basis regardless. The scope carve-out is correct; a backfill would only produce
  a red CI later.
...
- Negative: `re-artifacts/` gets no freshness coverage. Accepted: it is the correct
  semantics for immutable dated records. It is a settled choice, so there is nothing
  here to close later.
