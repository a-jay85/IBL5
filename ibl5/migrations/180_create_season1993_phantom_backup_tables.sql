-- Backup tables for the 1993 Warriors @ Supersonics phantom delete (migration 181).
-- Created empty and additively; filled and verified by 181 before any DELETE runs.
-- These tables are dedicated to the 1993 phantom backup and are NOT the 2008 rollback
...
-- because the 24 backed-up rows carry 24 distinct uuids, and migration 181's guard
-- makes a second population impossible.

CREATE TABLE IF NOT EXISTS ibl_box_scores_teams_season1993_phantom_backup LIKE ibl_box_scores_teams;
CREATE TABLE IF NOT EXISTS ibl_box_scores_season1993_phantom_backup       LIKE ibl_box_scores;
...
-- generated column cannot be written by an INSERT, so migration 181's explicit-column
...
ALTER TABLE ibl_box_scores_teams_season1993_phantom_backup
...
ALTER TABLE ibl_box_scores_season1993_phantom_backup
    MODIFY game_type     tinyint(3) unsigned NULL,
    MODIFY season_year   smallint(5) unsigned NULL,
    MODIFY calc_points   smallint(5) unsigned NULL,
