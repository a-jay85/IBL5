...
## Addendum — notify job and classifier coverage (2026-09-02)

Two facts recorded above describe the system as built on 2026-06-02. Both sentences are
left unchanged as the decision-time record; both have since moved on:

- **The `notify-inconclusive` job** in § A was later consolidated into a unified `notify`
  job together with the scheduled-failure notifier (ci-backlog 2.1). The
  `notify-ibl6-degradation` job it originally mirrored still exists in
  `.github/workflows/smoke-prod.yml`.
- **Classifier test coverage.** The Consequences bullet records an accepted limitation at
  decision time — the classifier shipped one-shot-verified rather than CI-gated, with a
  `bats` harness as a deferred follow-up. That follow-up landed in a different form: the
  exit-code scenarios are now covered by `ibl5/tests/Cli/SmokeProdCliTest.php` (PHPUnit,
  CI-gated via the Cli testsuite), not by `bats`.
