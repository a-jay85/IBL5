...
export const VR_MANIFEST: VrRow[] = [
  ...
  { name: 'free-agency', auth: 'auth', url: 'modules.php?name=FreeAgency',
    ...
    viewports: ['desktop', 'mobile'],
    dataDriven: true },
  { name: 'my-transactions', auth: 'auth', url: 'modules.php?name=MyTransactions',
    anchor: '.txn-table', viewports: ['desktop', 'mobile'],
    notes: 'CI user owns Metros (action.yml gm_username assignment), so the ledger renders the seed Metros stories deterministically; pending trade/FA sections render at seed state (no submission specs run in the VR job).' },
  { name: 'negotiation', auth: 'auth', url: 'modules.php?name=Player&pa=negotiate&pid=30',
    ...
    viewports: ['desktop', 'mobile'],
    notes: 'Player-module contract-extension page (NegotiationOfferView). pid=30 "Extension Vet" (Metros, owned by CI user) is renegotiation-eligible (cy=2, yr3 salary 0); the extension feature is gated off during Free Agency, so this row pins the in-season Regular-Season render. Locks the shared player-ratings table dedup (PR9 #6).' },
