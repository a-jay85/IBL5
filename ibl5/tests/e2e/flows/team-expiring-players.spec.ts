...
test.describe('Team page expiring-player fade', () => {
  ...
  withPhases(['Regular Season'], () => {
    test('expiring-contract rows absent during Regular Season', async ({ page }) => {
      await page.goto(TEAM_RATINGS_URL);
      await expect(page.locator('table.team-table')).toBeVisible();
      await expect(page.locator('table.team-table tbody tr.player-fa-expiring-row')).toHaveCount(0);
      await assertNoPhpErrors(page, TEAM_RATINGS_URL);
    });
  });
});
