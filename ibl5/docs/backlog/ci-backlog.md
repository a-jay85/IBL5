...
➜ 6.1 CI path-filter coupling splits sim-recap producer from consumer — ✅ Implemented (2026-08-08): see [archive](archive/ci-backlog-archive.md).
...
## Axis 8: Stale merge-queue premises in CI helper output (PR #1949)
...
## Axis 9: Deploy-path and review-quality spin-offs (PR #2044 audit)

Entries 9.1–9.3 are defects and gaps surfaced by the 2026-09-01 audit of PR #2044. Entry 9.1 is a fixed scp path bug; entries 9.2 and 9.3 are prevention-rule gaps (no code fix — rule doc owed). Entry 9.4 is a CI toolchain gap tracked here for completeness.
...
## Axis 12: Shallow-clone crash in CI tooling (PR #1950)

`bin/lighthouse-pr-urls` crashed on shallow CI checkouts, failing the Performance Audit job entirely. Entry 12.1 is the fixed crash; future fetch-depth decisions should treat this as a precedent.

| # | Title | Status | Automouse | Effort |
|---|-------|--------|-----------|-------:|
| 12.1 | `bin/lighthouse-pr-urls` crashes on shallow clones lacking `origin/master`, failing the Performance Audit CI job | ✅ fixed this pass | — | S |

### 12.1 `bin/lighthouse-pr-urls` crashes on shallow clones lacking `origin/master`
