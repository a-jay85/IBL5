...
J23 round-half-up + base_time re-center (✅, 2026-07-16, #1495) — coupled faithful fix deferred from J21; ADR-0085 records the hold finding; shipped round-half-up + baseTimeMid re-center 14.5→13.65 (J23)
  └─→ J24 possession-clock subsystem port (◑ Partial) — step classes + jitter, steal split + faithful shotValue2pt/3pt, matchupQuality Phase-3 matched (+0xDC8) & non-matched (+0x350, J25) terms, FG% band CLOSED via +0xD58 penalty-minutes (#1544), and the §1d steal-gating partition (#1547) all SHIPPED. **Gate-1 code-7 arming share is the SOLE cut-over blocker — NO-GO at 12.37% (floor 12.94, −0.57pp); next lever = armed-fraction / on-court-TransOff.** Open residuals: (7) 3P undershoot ~2.8pp, (6) `.plb dc_minutes` wiring, (3) CEngine+0x30 reader (with J17b writer), (5) baseTimeMid walkback. See the J24 entry for the full current-state + NOT-A-LEVER trap list
...
**Current state — CONSOLIDATED 2026-07-21** (dated measurement log in [archive](archive/jsb-native-backlog-archive.md)). This backlog is the authoritative record for J24; the `project_jsb_engine_frontier.md` memory keeps only the git-verified merged-PR hashes.

**⛔ SOLE cut-over blocker — gate-1 code-7 arming share NO-GO.** Measured on merged master (`84ff51085`; `TestFastClassArmingShareBaseline -tags archive`, recent-era 05-08, 97 snapshots, 85.77M possessions): **`DRBPushSharePct` = 12.37%** (10,608,830 code-7 / 75,158,396 half-court), band floor **12.94** → **0.57pp UNDER**. **Next lever = armed-fraction + on-court-`TransOff` composition** (real recent armed ~41.1% vs engine ~40.34% post-PR4b; on-court-5 mean picked TransOff ~6.15 vs ~5.658) — decomposed in `jsb-native/re-artifacts/jsb-J24-armed-transoff-RE-20260719.md`. **The on-court-`TransOff` sub-path via fatigue-sub *timing* is now CLOSED** (ceiling 12.61% < 12.94; see NOT-A-LEVER below), so the live levers are armed-fraction and a faithfulness-breaking `selectStarters` composition change. No constant has been tuned toward the band; scope shipped is "legible," not "GO."

**SHIPPED sub-work** (git-verified hashes in the frontier memory): step classes + half-court jitter + {3..23} redraw; the `FUN_004e4150` ratio retired (u=CEngine+0x38 binary-proved 0.0 → dead code, base_time a per-matchup constant); steal split (`stealTurnoverScale` 2.75e-5→1.69e-5 + non-arming `nonStealTurnover`, #1519) + faithful `shotValue2pt`/`shotValue3pt`; matchupQuality Phase-3 matched-defender term (+0xDC8, #1522) and non-matched `+0x350` term (#1527, J25); **FG% band CLOSED** via the `+0xD58` position-penalty base-minutes port (`penaltyBaseMinutes` = `dc>0?dc:MIN/GP`, #1544 — FG% 46.42→48.29% in [47.5,48.9], now a HARD assert); **§1d steal-gating partition** (#1547 — steal-armed possessions run the SAME single `transitionTriggers` gate as DRB via `gs.stealPushFired`; survivors merge into the code-7 `DRBPushClass`; ungated `StealClass` deleted → code-7 is now one band-comparable quantity on master).
...
**Do NOT re-open — settled / NOT-A-LEVER (each with its discriminating proof):**
...
**Residual (1) status:** partition complete (#1547); gate-1 re-adjudicated **NO-GO (12.37% < 12.94)**; next = armed-fraction / on-court-`TransOff` lever.
