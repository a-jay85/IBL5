...
- **`request.post()` and `page` are different PHP sessions.** A CSRF token read from the page is rejected by `request.post()` (cookies sync, session state does not). Submit CSRF-protected forms via `button.click()`; tamper hidden inputs with `evaluate()` if you need a different payload. Never hand a page token to `request.post()`.
