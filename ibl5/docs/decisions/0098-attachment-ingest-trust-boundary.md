...
- **Filenames never form a filesystem path.** The Discord filename is display-only (control chars stripped, length-capped). The on-disk name derives from `${message_id}-${attachment_id}.${ext}`, where both snowflakes are matched against `/^\d{1,20}$/` and the extension is mapped from `contentType` against a fixed allowlist (png/jpg/jpeg/gif/webp). A name like `../../evil.sh` still writes `${message_id}-${attachment_id}.png`.
...
- **`local_path` is full-shape-validated** before any DB write — a full-shape regex, not a prefix match, so `<root>/../../etc/passwd` is unrepresentable. Every DB value is bound; the only dynamic SQL is a placeholder count derived from `count($ids)`. Storage-layer idempotency is `INSERT IGNORE` + `UNIQUE (report_id, attachment_id)`, since the enqueue path is replay-safe.
- **The cache lives outside the repo tree** (`~/.claude/projects/.../bug-pipeline/attachments`) and is pruned at 7 days by the driver at tick start. The bot writer and the driver pruner share one absolute-path default, checked by a cross-process test row.
