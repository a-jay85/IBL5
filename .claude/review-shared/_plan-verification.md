...
- **The name must match the shipped declaration exactly.** Write the bare method name — no class prefix, no `()`, no `::`.- **Fenced examples do not count.** Both parsers strip fenced blocks before reading the section, so an illustrative list inside a fence yields zero entries.
...
| Plan adds or modifies `htmx:beforeRequest` / `htmx:afterRequest` handlers that mutate DOM state (element disabled/enabled, text changed) | Only a real browser navigation catches the htmx history-cache snapshot trap (see `.claude/rules/htmx-history-cache.md`) |
| Plan changes a PHP endpoint's HTTP response shape (PRG redirect, inline-render to JSON, or removes an E2E-tested response branch) | Audit E2E tests for that endpoint; update assertions that depended on the old response shape. |
...
| adds or modifies an updater or importer that reads a generated file from a repo-relative path | asserts the source path is NOT git-tracked: `git ls-files --error-unmatch <path>` exits nonzero (untracked — expected) |
...
| adds or modifies an importer that writes records to a secondary/audit table (e.g., a junction or tracking table alongside a primary entity table) | carries an integration test that reads the canonical flag column in the primary table after a complete import cycle |
| adds or modifies a detection check in an audit class that has a fail-open guard conditioned on data availability (e.g., `$isEnabled = $dataIndex !== []`) | asserts the check fires even when the guard-controlling condition is false |
| adds or modifies a `proc_open` call site in a PHP CLI script or class | asserts that `proc_close` is called and its return value checked (non-zero exit → error path), that any stderr pipe descriptor is fully drained before `proc_close`, and that command-line delimiters (e.g., `-z` NUL for `git check-ignore`) match the parser on the receiving end |
| adds or modifies a schema validator for LLM output in the post-plan harness (`tools/postplan-harness`) | asserts that a common envelope variation of the expected shape (e.g. `{"<key>": [...]}` wrapping a bare array) degrades gracefully — either unwrapped by a normalizer or caught as a degraded-state — and does NOT post a false safety signal (`"No issues found"` / `"No security issues found"`) |
| modifies an `echo` or log string in a shell script that is already asserted by a numeric-token `grep` (e.g. `grep -qF '#<N>'`) in an existing test | adds a full-wording assertion that pins the exact rendered log line (including its behavioral claim), not merely the numeric token |
| adds or modifies a port-guard or process-detection guard that pipes lsof output to a pattern filter (grep, awk) | asserts the "port free" case exits with the expected free-port verdict |
